...
class $Posts {
  static async load(env: Env): Promise<Posts.Post[]> {
  ...
  static find(posts: Posts.Post[], slug: string): Posts.Post | undefined {
  ...
  static summaries(posts: Posts.Post[]): Posts.PostSummary[] {
    return posts.map(
      ({
