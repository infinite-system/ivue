...
class $CheckStandard {
  ...
  static readonly SETUP_STATE_CALLS = new Set([
    'ref',
    'shallowRef',
    'reactive',
    'computed',
  ...
  static readonly LIFECYCLE_HOOKS = new Set([
    'onMounted',
    'onUnmounted',
    'onBeforeMount',
    'onBeforeUnmount',
  ...
  static get $grammar() {
    ...
    return {
      ...
      RECORD_LINE: new RegExp(
        `(?<![\\w-])${RECORD}:\\s*([^(\\n]+?)\\s*\\(([^)\\n]*\\.${RECORD}s\\.md)\\)`
      ),
      CONTRACT_LINK: new RegExp(
        `\\[([^\\]]+)\\]\\(([^)\\s]*\\.${RECORD}s\\.md)(#[^)\\s]*)?\\)`,
        'g'
      )
    };
  ...
  static get exactly_one_reactive_source_is_installed(): CheckStandard.StandardCheck {
    return this.defineCheck('exactly_one_reactive_source_is_installed', (context) => {
      const vendored = context.sources.filter(
        (unit) =>
          /export\s+function\s+Reactive\s*[<(]/.test(unit.text) ||
          /export\s*\{[^}]*\bReactive\b[^}]*\}\s*from/.test(unit.text)
      );
      ...
      for (const manifest of manifests) {
        ...
        if (parsed.name === 'ivue' || parsed.dependencies?.ivue || parsed.devDependencies?.ivue)
          dependency = true;
      }
  ...
  static get a_public_class_publishes_its_namespace_manifest(): CheckStandard.StandardCheck {
    return this.defineCheck('a_public_class_publishes_its_namespace_manifest', (context) => {
      ...
      const isBehavioralObject = (expression: ts.Expression | undefined): boolean => {
        ...
        return (
          ts.isObjectLiteralExpression(bare) &&
          bare.properties.some(
            (property) =>
              ts.isMethodDeclaration(property) ||
              (ts.isPropertyAssignment(property) && this.isFunctionLike(property.initializer))
          )
        );
      };
          ...
          if (!namespace) {
            findings.push(
              this.finding(
                ...
                unit,
                line,
                `class ${classFile.rawName} has no \`export namespace ${publicName}\``
              )
            );
            continue;
          }
          if (!anchorInitializer)
            findings.push(
              this.finding(
                this.a_public_class_publishes_its_namespace_manifest,
                unit,
                this.lineOf(unit, namespace),
                `namespace ${publicName} lacks \`export const $Class = …\``
              )
            );
          if (!classInitializer)
            findings.push(
              this.finding(
                ...
                unit,
                this.lineOf(unit, namespace),
                `namespace ${publicName} lacks \`export let Class = …\``
              )
            );
          if (isReactive && !hasInstanceType)
            findings.push(
              this.finding(
                ...
                unit,
                this.lineOf(unit, namespace),
                `reactive namespace ${publicName} lacks \`export type Instance = typeof Class.Instance\``
              )
            );
          continue;
        }
          ...
          if (ts.isClassDeclaration(statement) && exported)
            findings.push(
              this.finding(
                ...
                'a class is exported directly — publish `$X` through `export namespace X`'
              )
            );
          if (ts.isExportAssignment(statement) && isBehavioralObject(statement.expression))
            findings.push(
              this.finding(
                ...
                unit,
                this.lineOf(unit, statement),
                'a behavioral object is exported directly — behavior belongs to a namespace Static class'
              )
            );
  ...
  static get a_class_file_is_named_after_its_class(): CheckStandard.StandardCheck {
    return this.defineCheck('a_class_file_is_named_after_its_class', (context) => {
      ...
      for (const unit of context.sources) {
        ...
        // `MessagePart.Text.ts` declares `$MessagePartText`: a dot in the stem is the kit path
        // (compositor, then role), and the class name is the stem with its dots removed.
        const stem = basename(unit.path).replace(/\.ts$/, '').replace(/\./g, '');
        if (stem !== classFile.publicName)
          findings.push(
            this.finding(
              this.a_class_file_is_named_after_its_class,
              unit,
              this.lineOf(unit, classFile.rawClass),
              `file \`${stem}.ts\` declares \`${classFile.rawName}\` — the file, class and namespace share one name`
            )
          );
      }
      return findings;
      ...
      (context) => {
        ...
        for (const unit of context.sources) {
          ...
          for (const statement of unit.ast.statements) {
              ...
              if (!seenClass)
                findings.push(
                  this.finding(
                    ...
                    unit,
                    this.lineOf(unit, statement),
                    `namespace ${classFile.publicName} precedes its class ${classFile.rawName}`
                  )
                );
              continue;
            }
            ...
            if (ts.isInterfaceDeclaration(statement) && statement.name.text === classFile.rawName)
              continue;
            if (
              ...
              findings.push(
                this.finding(
                  ...
                  unit,
                  this.lineOf(unit, statement),
                  `\`${statement.name.text}\` is a type outside the namespace — every type a class file declares is a member of \`namespace ${classFile.publicName}\` (export type / interface), read as \`${classFile.publicName}.${statement.name.text}\``
                )
              );
              continue;
            }
        ...
        for (const statement of classFile.namespace.body.statements) {
          ...
          if (ts.isFunctionDeclaration(statement)) {
            findings.push(
              this.finding(
                ...
                unit,
                this.lineOf(unit, statement),
                `\`${statement.name?.text ?? 'function'}\` is behavior in namespace ${classFile.publicName} — move it onto the class as a static method`
              )
            );
            continue;
          }
          findings.push(
            this.finding(
              ...
              unit,
              this.lineOf(unit, statement),
              `namespace ${classFile.publicName} holds a runtime statement — the namespace is \`$Class\`, \`Class\` and types only`
            )
          );
        }
      }
          ...
          if (ts.isPropertyDeclaration(member) && this.isFunctionLike(member.initializer))
            findings.push(
              this.finding(
                ...
                unit,
                this.lineOf(unit, member),
                `\`${this.memberName(member)}\` is a function-valued field — write it as a method; the engine binds methods lazily`
              )
            );
        }
      }
            ...
            if (ts.isIdentifier(callee) && callee.text.startsWith('$'))
              findings.push(
                this.finding(
                  ...
                  unit,
                  this.lineOf(unit, node),
                  `\`new ${callee.text}()\` constructs the raw class — construct \`${callee.text.slice(1)}.Class\``
                )
              );
            if (ts.isPropertyAccessExpression(callee) && callee.name.text === '$Class')
              findings.push(
                this.finding(
                  ...
                  unit,
                  this.lineOf(unit, node),
                  `\`new ${callee.getText(unit.ast)}()\` constructs the anchor — construct \`.Class\``
                )
              );
          }
          ...
          if (
            ...
            findings.push(
              this.finding(
                ...
                unit,
                this.lineOf(unit, node),
                '`reactive(new …)` wraps an instance without the Instance cast — instances are raw; a reactive() view is `reactive(new X.Class() as X.Instance)`'
              )
            );
        });
      }
  ...
  static get the_anchor_is_static_only_when_statics_exist(): CheckStandard.StandardCheck {
    return this.defineCheck('the_anchor_is_static_only_when_statics_exist', (context) => {
      ...
      for (const unit of context.sources) {
        ...
        const hasStatics = classFile.rawClass.members.some(
          (member) => this.isStaticMember(member) && !ts.isConstructorDeclaration(member)
        );
        ...
        if (hasStatics && !classFile.isStaticAnchored)
          findings.push(
            this.finding(
              ...
              unit,
              anchorLine,
              `${classFile.rawName} declares statics but the anchor is raw — \`export const $Class = Static(${classFile.rawName})\``
            )
          );
        if (!hasStatics && classFile.isStaticAnchored)
          findings.push(
            this.finding(
              ...
              unit,
              anchorLine,
              `${classFile.rawName} declares no statics but the anchor is \`Static(…)\` — \`export const $Class = ${classFile.rawName}\``
            )
          );
      }
      return findings;
  ...
  static get static_binds_methods_and_caches_dollar_getters_per_receiver(): CheckStandard.StandardCheck {
    return this.defineCheck(
      ...
      (context) => {
        ...
        const probe = (message: string): CheckStandard.Finding => ({
          check: 'static_binds_methods_and_caches_dollar_getters_per_receiver',
          file: 'ivue/extras',
          line: 0,
          message: `${message} (probed from ${unit?.relativePath ?? 'the gate'})`
        });
        const StaticUnderTest = context.staticImplementation;
        if (!StaticUnderTest)
          return [
            probe('`Static` could not be loaded from ivue/extras — the runtime probe did not run')
          ];
        const findings: CheckStandard.Finding[] = [];
        let cacheRuns = 0;
        class $Probe {
        ...
        if (detached !== Anchor.method)
          findings.push(probe('`Static` does not keep method identity stable across reads'));
        if (detached() !== Anchor)
          findings.push(probe('`Static` does not bind static methods to the receiving class'));
        const first = Anchor.$cache;
        if (first !== Anchor.$cache || cacheRuns !== 1)
          findings.push(probe('`Static` does not cache a dollar getter once per receiver'));
        class Sub extends Anchor {}
        const subCache = Sub.$cache;
        if (subCache === first || subCache.receiver !== Sub || cacheRuns !== 2)
          findings.push(probe('`Static` lets a parent dollar-cache shadow a subclass receiver'));
        if (Sub.method() !== Sub)
          findings.push(probe('`Static` binds a subclass method to the parent'));
        return findings;
      }
    );
  }
    ...
    return this.defineCheck('a_shared_store_is_a_static_readonly_field', (context) => {
      ...
      for (const unit of context.sources) {
        ...
        for (const member of classFile.rawClass.members) {
          ...
          const storeShaped = ts.isNewExpression(initializer)
            ? !!initializer.expression &&
              ts.isIdentifier(initializer.expression) &&
              ['Map', 'Set', 'WeakMap', 'WeakSet', 'Array'].includes(initializer.expression.text)
          ...
          if (storeShaped && !this.isReadonlyMember(member))
            findings.push(
              this.finding(
                ...
                unit,
                this.lineOf(unit, member),
                `static \`${this.memberName(member)}\` is a mutable shared store — declare it \`static readonly\``
              )
            );
          ...
          if (constructsNamespaceClass)
            findings.push(
              this.finding(
                ...
                unit,
                this.lineOf(unit, member),
                `static \`${this.memberName(member)}\` constructs another namespace's class at module load — hold it in \`new LazyShared(() => …)\``
              )
            );
        }
      }
    ...
    return this.defineCheck('a_derived_static_getter_is_lower_camel_case', (context) => {
      ...
      const isLiteral = (expression: ts.Expression): boolean => {
        ...
        if (
          ...
          return isLiteral(expression.operand);
        if (ts.isBinaryExpression(expression))
          return isLiteral(expression.left) && isLiteral(expression.right);
        ...
        if (ts.isArrayLiteralExpression(expression))
          return expression.elements.every(
            (element) => ts.isExpression(element) && isLiteral(element)
          );
        if (ts.isObjectLiteralExpression(expression))
          return expression.properties.every(
            (property) => ts.isPropertyAssignment(property) && isLiteral(property.initializer)
          );
        ...
        if (ts.isPropertyAccessExpression(expression))
          return (
            /^[A-Z][A-Z0-9_]*$/.test(expression.name.text) && expression.name.text.includes('_')
          );
        return false;
      };
      for (const unit of context.sources) {
        ...
        if (!classFile) continue;
        for (const member of classFile.rawClass.members) {
          if (!ts.isGetAccessorDeclaration(member) || !this.isStaticMember(member) || !member.body)
            continue;
          ...
          const camel = name
            .toLowerCase()
            .replace(/_(\w)/g, (whole, letter: string) => letter.toUpperCase());
          findings.push(
            this.finding(
              this.a_derived_static_getter_is_lower_camel_case,
              unit,
              this.lineOf(unit, member),
              `static get ${name}() derives its value — a derived getter is lowerCamel (\`${camel}\`); SCREAMING_SNAKE is for tunable constants: literals, or other SCREAMING constants composed`
            )
          );
        }
      }
        ...
        for (const member of classFile.rawClass.members) {
          ...
          this.forEachDescendant(member, (node) => {
            if (
              ...
              ts.isIdentifier(node.expression) &&
              node.expression.text === classFile.rawName
            )
              findings.push(
                this.finding(
                  ...
                  unit,
                  this.lineOf(unit, node),
                  `\`${node.getText(unit.ast)}\` pins the read to the base class — read \`this.self.${node.name.text}\``
                )
              );
            if (
              ...
              !(ts.isGetAccessorDeclaration(member) && this.memberName(member) === 'self')
            )
              findings.push(
                this.finding(
                  this.static_reads_go_through_self_not_the_base_class,
                  unit,
                  this.lineOf(unit, node),
                  'per-site `this.constructor as …` cast — declare `get self()` once and read through it'
                )
              );
          });
        }
      ...
      for (const unit of context.sources) {
        ...
        for (const member of classFile.rawClass.members) {
          if (
            !ts.isPropertyDeclaration(member) ||
            this.isStaticMember(member) ||
            this.isReadonlyMember(member) ||
            this.isDeclaredMember(member)
          )
            continue;
          const initializer = member.initializer;
          const fromFactory =
          ...
          findings.push(
            this.finding(
              ...
              unit,
              this.lineOf(unit, member),
              `\`${this.memberName(member)}\` is a mutable plain field — writes trigger nothing; declare \`get ${this.memberName(member)}() { return ref(…) }\``
            )
          );
        }
      }
      ...
      for (const unit of context.sources) {
        ...
        this.forEachDescendant(classFile.rawClass, (node) => {
          ...
          if (
            ...
            refGetters.has(target.name.text)
          )
            findings.push(
              this.finding(
                ...
                unit,
                this.lineOf(unit, node),
                `\`this.${target.name.text} = …\` assigns over a Ref getter — write \`this.${target.name.text}.value = …\``
              )
            );
        });
      }
      ...
      (context) => {
        ...
        for (const unit of context.sources) {
          ...
          for (const member of classFile.rawClass.members) {
            ...
            const justified = this.COMPUTED_JUSTIFICATIONS.some((category) =>
              leading.includes(category)
            );
            if (!justified)
              findings.push(
                this.finding(
                  ...
                  unit,
                  this.lineOf(unit, member),
                  `\`get ${this.memberName(member)}()\` allocates a computed without a stated reason — derive with a plain getter, or justify above it: \`// computed: expensive | render-suppression | stable-handle\``
                )
              );
          }
        }
      ...
      for (const unit of context.sources) {
        ...
        for (const member of classFile.rawClass.members) {
          if (
            ...
            /^use[A-Z]/.test(member.initializer.expression.text)
          )
            findings.push(
              this.finding(
                ...
                unit,
                this.lineOf(unit, member),
                `\`${this.memberName(member)} = ${member.initializer.expression.text}()\` runs at construction — inject it as \`protected get $${this.memberName(member)}() { return ${member.initializer.expression.text}() }\``
              )
            );
          ...
          if (
            ...
            !this.isStaticMember(member) &&
            this.memberName(member).startsWith('$') &&
            member.body
          ) {
            ...
            const single =
              ...
              (ts.isCallExpression(statements[0].expression) ||
                ts.isNewExpression(statements[0].expression) ||
                ts.isPropertyAccessExpression(statements[0].expression));
            if (!single)
              findings.push(
                this.finding(
                  this.a_composable_is_injected_by_a_one_call_dollar_getter,
                  unit,
                  this.lineOf(unit, member),
                  `\`get ${this.memberName(member)}()\` does more than one call — a dollar getter creates its singleton and nothing else`
                )
              );
          }
        }
          ...
          if (ts.isTypeReferenceNode(node)) {
            ...
            if (tail?.member === 'Instance') {
              ...
              const inContainer =
                ...
                RAW_CONTAINERS.has(parent.typeName.text);
              const asParameter = ts.isParameter(parent) && !ts.isArrowFunction(parent.parent);
              if (inArray || inContainer || asParameter)
                report(
                  this.lineOf(unit, node),
                  `\`${tail.namespace}.Instance\` types a raw graph position (collection, ref, or parameter) — raw instances are \`${tail.namespace}.Model\`; \`Instance\` is for unwrapping surfaces only`
                );
            }
            if (tail?.member === 'Model') {
              ...
              const inUnwrap =
                (ts.isAsExpression(parent) &&
                  ...
                  ['defineExpose', 'reactive'].includes(parent.parent.expression.text)) ||
                (ts.isTypeReferenceNode(parent) &&
                  ts.isIdentifier(parent.typeName) &&
                  parent.typeName.text === 'ShallowUnwrapRef');
              if (inUnwrap)
                report(
                  this.lineOf(unit, node),
                  `\`${tail.namespace}.Model\` on an unwrapping surface — type it \`${tail.namespace}.Instance\` (strips readonly so ref writes typecheck)`
                );
            }
          }
          if (
            ...
            report(
              this.lineOf(unit, node),
              `\`defineExpose(${node.arguments[0].text})\` exposes the raw type — \`defineExpose(${node.arguments[0].text} as X.Instance)\``
            );
        });
      };
      for (const unit of context.sources)
        inspect(unit, (line, message) =>
          findings.push(
            this.finding(this.instance_types_only_unwrapping_surfaces, unit, line, message)
          )
        );
      for (const component of context.components)
        if (component.script)
          inspect(component.script, (line, message) =>
            findings.push(
              this.componentFinding(
                this.instance_types_only_unwrapping_surfaces,
                component,
                line + component.scriptLine - 1,
                message
              )
            )
          );
  ...
  static get a_component_has_one_model_owner(): CheckStandard.StandardCheck {
    return this.defineCheck('a_component_has_one_model_owner', (context) => {
      ...
      for (const component of context.components) {
        ...
        for (const extra of constructions.slice(1))
          findings.push(
            this.componentFinding(
              this.a_component_has_one_model_owner,
              component,
              this.componentLine(component, extra.node),
              `a second model is constructed (\`${extra.variable}\`) — one template, one logic owner`
            )
          );
      }
      return findings;
        ...
        for (const statement of component.script.ast.statements) {
          if (ts.isFunctionDeclaration(statement))
            findings.push(
              this.componentFinding(
                ...
                component,
                this.componentLine(component, statement),
                `free function \`${statement.name?.text ?? ''}\` beside the model — behavior belongs on the class as a method`
              )
            );
          this.forEachDescendant(statement, (node) => {
            if (
              !ts.isCallExpression(node) ||
              !ts.isIdentifier(node.expression) ||
              !this.SETUP_STATE_CALLS.has(node.expression.text)
            )
              return;
            ...
            findings.push(
              this.componentFinding(
                this.script_setup_is_wiring_only,
                component,
                this.componentLine(component, node),
                `\`${node.expression.text}()\` in \`<script setup>\` — component-local reactive behavior beside the class; state, derivations and watchers live in the class`
              )
            );
          });
        }
      ...
      for (const component of context.components) {
        ...
        this.forEachDescendant(component.script.ast, (node) => {
          if (
            !ts.isCallExpression(node) ||
            !ts.isIdentifier(node.expression) ||
            !this.LIFECYCLE_HOOKS.has(node.expression.text)
          )
            return;
          if (this.isInsideClassBody(node)) return;
          ...
          findings.push(
            this.componentFinding(
              ...
              component,
              this.componentLine(component, node),
              `\`${node.expression.text}()\` in \`<script setup>\` carries logic — a hook may only delegate one call to the model (\`${node.expression.text}(() => model.method())\`); logic lives in a method`
            )
          );
        });
      }
      ...
      for (const component of context.components) {
        ...
        for (const construction of this.modelConstructions(component)) {
          ...
          for (const member of classFile.rawClass.members) {
            if (this.isStaticMember(member)) continue;
            const name = this.memberName(member);
            if (ts.isGetAccessorDeclaration(member) && !refGetters.has(name))
              plainGetters.add(name);
          ...
          for (const statement of component.script.ast.statements) {
            ...
            for (const declaration of statement.declarationList.declarations) {
              if (
                ...
                declaration.initializer.text !== construction.variable
              )
                continue;
              for (const element of declaration.name.elements) {
                ...
                if (plainGetters.has(source))
                  findings.push(
                    this.componentFinding(
                      this.the_state_destructure_is_total,
                      component,
                      line,
                      `\`${source}\` is a plain getter — destructuring snapshots a dead value; read \`${construction.variable}.${source}\` dotted`
                    )
                  );
                if (methods.has(source))
                  findings.push(
                    this.componentFinding(
                      this.the_state_destructure_is_total,
                      component,
                      line,
                      `\`${source}\` is a method — keep it dotted (\`${construction.variable}.${source}()\`) unless a profiled hot path says otherwise`
                    )
                  );
                if (props.has(bound))
                  findings.push(
                    this.componentFinding(
                      ...
                      component,
                      line,
                      `state binding \`${bound}\` shadows the prop of the same name in the template`
                    )
                  );
              }
            }
          ...
          for (const expression of component.expressions) {
            ...
            this.forEachDescendant(parsed, (node) => {
              if (
                ts.isPropertyAccessExpression(node) &&
                ts.isIdentifier(node.expression) &&
                node.expression.text === construction.variable &&
                refGetters.has(node.name.text)
              )
                findings.push(
                  this.componentFinding(
                    this.the_state_destructure_is_total,
                    component,
                    expression.line,
                    `\`${construction.variable}.${node.name.text}\` reaches a Ref through the instance (always truthy in \`v-if\`) — destructure \`${node.name.text}\` as a state binding`
                  )
                );
            });
          }
  ...
  static get one_handler_per_event(): CheckStandard.StandardCheck {
    return this.defineCheck('one_handler_per_event', (context) => {
      ...
      const visit = (
        ...
        component: CheckStandard.ComponentUnit,
        offset: number
      ) => {
          ...
          for (const [handler, events] of handlers) {
            ...
            findings.push(
              this.componentFinding(
                ...
                component,
                element.loc.start.line + offset,
                `\`@${events.join('` and `@')}\` bind one handler (\`${handler}\`) — one DOM event, one handler named for the event: give each its own method, a one-line delegate where they coincide, so a subclass can extend either alone`
              )
            );
          }
    ...
    return this.defineCheck('template_expressions_carry_no_logic', (context) => {
      ...
      const isNamedRead = (expression: ts.Expression): boolean => {
        ...
        if (ts.isCallExpression(expression))
          return (
            isNamedRead(expression.expression) &&
            expression.arguments.every(
              (argument) =>
                isNamedRead(argument) ||
          ...
          this.forEachDescendant(parsed, (node) => {
            ...
            findings.push(
              this.componentFinding(
                ...
                component,
                expression.line,
                `${what} in the template (\`${expression.code.trim().slice(0, 60)}\`) — name it as a plain getter (or a method when it takes an argument)`
              )
            );
          });
        }
  ...
  static get watch_lifetime_matches_the_instance_owner(): CheckStandard.StandardCheck {
    return this.defineCheck('watch_lifetime_matches_the_instance_owner', (context) => {
      ...
      for (const component of context.components)
        for (const construction of this.modelConstructions(component))
          componentScoped.add(construction.namespace);
      ...
      for (const unit of context.sources) {
        ...
        this.forEachDescendant(unit.ast, (node) => {
          ...
          if (classFile && ancestor === classFile.rawClass) {
            if (!constructedInside.has(classFile.publicName))
              constructedInside.set(classFile.publicName, new Set());
            constructedInside.get(classFile.publicName)!.add(constructed);
          } else constructedElsewhere.add(constructed);
        });
        if (
          classFile?.namespace?.body &&
          ts.isModuleBlock(classFile.namespace.body) &&
          classFile.namespace.body.statements.some(
            (statement) => ts.isFunctionDeclaration(statement) && statement.name?.text === 'use'
          )
        )
          constructedElsewhere.add(classFile.publicName);
        if (
          classFile &&
          classFile.rawClass.members.some(
            (member) =>
              ts.isMethodDeclaration(member) &&
              this.isStaticMember(member) &&
              this.memberName(member) === 'use'
          )
        )
          constructedElsewhere.add(classFile.publicName);
      }
      ...
      while (grew) {
        ...
        for (const [host, constructed] of constructedInside) {
          ...
          for (const name of constructed)
            if (!componentScoped.has(name)) {
              componentScoped.add(name);
              grew = true;
            }
        }
      }
      ...
      for (const [host, constructed] of constructedInside)
        if (!componentScoped.has(host)) for (const name of constructed) outliving.add(name);
      for (const unit of context.sources) {
        const classFile = this.classFileOf(unit);
        if (!classFile) continue;
        ...
        if (isComponentScoped && usesDollarWatch && !hasScopeBridge)
          findings.push(
            this.finding(
              this.watch_lifetime_matches_the_instance_owner,
              unit,
              dollarLine,
              `${classFile.rawName} is constructed in a component's setup but uses \`this.$watch\` — its scope would outlive unmount; use plain \`watch\` (the component scope reaps it)`
            )
          );
        if (isOutliving && usesPlainWatch)
          findings.push(
            this.finding(
              this.watch_lifetime_matches_the_instance_owner,
              unit,
              plainLine,
              `${classFile.rawName} outlives components (constructed outside setup) but uses plain \`watch\` — there is no component scope to reap it; use \`this.$watch\``
            )
          );
        if (usesDollarWatch && !hasDisposePath)
          findings.push(
            this.finding(
              this.watch_lifetime_matches_the_instance_owner,
              unit,
              dollarLine,
              `${classFile.rawName} registers \`$watch\` effects but has no dispose path — call \`$stopEffects()\` from an owner method, or auto-wire \`onScopeDispose\``
            )
          );
      }
      return findings;
    ...
    return this.defineCheck('a_reactive_closure_delegates_to_one_method', (context) => {
      ...
      for (const unit of context.sources) {
        ...
        this.forEachDescendant(classFile.rawClass, (node) => {
          ...
          } else if (calleeName.endsWith('watch') && node.arguments[1])
            callbacks.push(node.arguments[1]);
          else if (calleeName.endsWith('watchEffect') && node.arguments[0])
            callbacks.push(node.arguments[0]);
          for (const callback of callbacks) {
            if (
              ts.isPropertyAccessExpression(callback) &&
              callback.expression.kind === ts.SyntaxKind.ThisKeyword
            ) {
              findings.push(
                this.finding(
                  this.a_reactive_closure_delegates_to_one_method,
                  unit,
                  this.lineOf(unit, callback),
                  `\`${calleeName}(${callback.getText(unit.ast)})\` passes the method directly — use the arrow form \`() => ${callback.getText(unit.ast)}()\``
                )
              );
              continue;
            }
            if (!this.delegateCall(callback))
              findings.push(
                this.finding(
                  ...
                  unit,
                  this.lineOf(unit, callback),
                  `${calleeName} callback carries logic — delegate to one method: \`() => this.method(…)\``
                )
              );
          }
        });
      ...
      for (const unit of context.sources) {
        this.forEachDescendant(unit.ast, (node) => {
          if (
            ...
            !this.isInsideFunctionBody(node)
          )
            findings.push(
              this.finding(
                ...
                unit,
                this.lineOf(unit, node),
                `\`${node.getText(unit.ast)}\` constructs a singleton at module load — publish it behind \`use()\` (\`singleton ??= new Class()\`) so it constructs on first touch and tests can swap the \`Class\` slot first`
              )
            );
          if (ts.isParameter(node) && node.type && ts.isConstructorDeclaration(node.parent)) {
            ...
            if (
              tail &&
              (tail.member === 'Instance' || tail.member === 'Model') &&
              ts.isIdentifier(node.name) &&
              /^(app|store|session|root|shell)$/i.test(node.name.text)
            )
              findings.push(
                this.finding(
                  this.a_store_is_used_lazily_and_swapped_at_the_class_slot,
                  unit,
                  this.lineOf(unit, node),
                  `constructor takes the shared model \`${node.name.text}: ${tail.namespace}.${tail.member}\` — reach for it with \`protected get $${node.name.text}() { return ${tail.namespace}.use() }\``
                )
              );
          }
        });
      ...
      for (const component of context.components) {
        ...
        this.forEachDescendant(component.script.ast, (node) => {
          ...
          for (const member of typeArgument.members) {
            ...
            const storeShaped =
              /^(app|store|session|root|shell)$/i.test(member.name.text) ||
              /Store$/.test(tail?.namespace ?? '');
            if (tail && (tail.member === 'Instance' || tail.member === 'Model') && storeShaped)
              findings.push(
                this.componentFinding(
                  this.a_store_is_used_lazily_and_swapped_at_the_class_slot,
                  component,
                  this.componentLine(component, member),
                  `prop \`${member.name.text}: ${tail.namespace}.${tail.member}\` drills a shared model — a store is reached with \`${tail.namespace}.use()\`, never passed down`
                )
              );
          }
        });
      ...
      for (const unit of context.sources) {
        ...
        for (const member of classFile.rawClass.members) {
          ...
          for (const method of classFile.rawClass.members) {
            ...
            if (
              /^(?:set|write|bump|update|put|apply|invalidate)/.test(this.memberName(method)) &&
              new RegExp(`this\\.${overlay}\\.set\\(`).test(body)
            )
              writers.push(method);
          }
          if (!releases)
            findings.push(
              this.finding(
                ...
                unit,
                this.lineOf(unit, member),
                `keyed overlay \`${overlay}\` has no release path — no method deletes or clears its entries; a Map of refs cannot GC on its own`
              )
            );
          for (const writer of writers)
            findings.push(
              this.finding(
                this.keyed_state_creates_on_read_and_peeks_on_write,
                unit,
                this.lineOf(unit, writer),
                `write path \`${this.memberName(writer)}\` creates entries in \`${overlay}\` — writes PEEK (\`get(key)?.value++\`); only reads get-or-create`
              )
            );
        }
      }
  ...
  static get a_generic_reactive_class_casts_its_constructor(): CheckStandard.StandardCheck {
    return this.defineCheck('a_generic_reactive_class_casts_its_constructor', (context) => {
      ...
      for (const unit of context.sources) {
        ...
        if (
          !classFile?.namespace?.body ||
          !classFile.rawClass.typeParameters?.length ||
          !classFile.isReactive
        )
          continue;
        ...
        if (!/as\s+unknown\s+as\s+typeof\s+\$\w+/.test(classText))
          findings.push(
            this.finding(
              ...
              unit,
              this.lineOf(unit, classFile.classInitializer ?? classFile.namespace),
              `generic ${classFile.rawName}: \`Class\` erases <T> — \`export let Class = Reactive($Class) as unknown as typeof $Class\``
            )
          );
        const instanceAlias = ts.isModuleBlock(classFile.namespace.body)
          ? classFile.namespace.body.statements.find(
              (statement): statement is ts.TypeAliasDeclaration =>
                ts.isTypeAliasDeclaration(statement) && statement.name.text === 'Instance'
            )
          : undefined;
        if (
          instanceAlias &&
          (!instanceAlias.typeParameters?.length ||
            !/ReactiveInstance\s*</.test(instanceAlias.type.getText(unit.ast)))
        )
          findings.push(
            this.finding(
              this.a_generic_reactive_class_casts_its_constructor,
              unit,
              this.lineOf(unit, instanceAlias),
              `generic ${classFile.rawName}: \`Instance\` must carry <T> and apply ReactiveInstance by hand — \`export type Instance<T> = ReactiveInstance<${classFile.rawName}<T>>\``
            )
          );
      }
      return findings;
      ...
      for (const unit of context.sources) {
        ...
        this.forEachDescendant(unit.ast, (node) => {
          ...
          if (node.name.text === '$Class') {
            ...
            while (
              heritage &&
              (ts.isAsExpression(heritage) || ts.isParenthesizedExpression(heritage))
            )
          ...
          findings.push(
            this.finding(
              ...
              unit,
              this.lineOf(unit, node),
              `\`${node.getText(unit.ast)}\` is read at module evaluation — read it inside a getter or method body (any load order then resolves)`
            )
          );
        });
      }
          ...
          if (name === '_' || single || banned)
            findings.push(
              this.finding(
                ...
                unit,
                this.lineOf(unit, identifier),
                `\`${name}\` — unfold to the domain word (row, cell, newValue, event…); single letters and abbreviations are not names`
              )
            );
        });
      };
      ...
      for (const component of context.components) {
        for (const alias of component.forAliases) {
          for (const name of alias.names) {
            ...
            if (
              name === '_' ||
              (bare.length === 1 && !this.DOMAIN_TERMS.has(bare)) ||
              this.BANNED_NAMES.has(bare.toLowerCase())
            )
              findings.push(
                this.componentFinding(
                  this.declarations_use_full_descriptive_names,
                  component,
                  alias.line,
                  `\`${name}\` as a \`v-for\` alias — unfold to the domain word (row, cell, column, index…); single letters and abbreviations are not names`
                )
              );
          }
        }
      ...
      for (const unit of context.sources) {
        ...
        for (const member of classFile.rawClass.members) {
          ...
          if (currentRank < highest)
            findings.push(
              this.finding(
                ...
                unit,
                this.lineOf(unit, member),
                `${rankName[currentRank]} follows ${rankName[highest]} — order is statics, constructor, getters, methods`
              )
            );
          ...
          if (previous && ts.isMethodDeclaration(member) && ts.isMethodDeclaration(previous)) {
            ...
            const between = unit.lines.slice(
              previousEndLine + 1,
              unit.ast.getLineAndCharacterOfPosition(member.getStart(unit.ast)).line
            );
            if (!between.some((line) => line.trim() === '') && startLine >= previousEndLine)
              findings.push(
                this.finding(
                  ...
                  unit,
                  this.lineOf(unit, member),
                  `method \`${this.memberName(member)}\` is not separated from the previous method by a blank line — methods are paragraphs`
                )
              );
          }
          previous = member;
...
`;
  ...
  static get proofs(): Readonly<Record<string, CheckStandard.CheckProof>> {
    ...
    const pointerTest = (pointer: string, annotation: string) =>
      fixture.validTest
        ...
        .replace(
          `// ${grammar.IMPOSSIBLE}: $Box — height decreases without a grow call\ntest('height never decreases on its own'`,
          `${annotation}// ${grammar.IMPOSSIBLE}: $Box — height decreases without a grow call\ntest('height never decreases on its own'`
        );
    return {
      exactly_one_reactive_source_is_installed: {
        claim:
          'If the gate runs over a checkout, then it finds exactly one engine, an ivue dependency or one vendored Reactive, never zero and never two',
        ...
        red: [
          {
            files: {
              ...box,
              'src/Reactive.ts':
                'export function Reactive<C>(targetClass: C): C { return targetClass; }\n'
            },
      ...
      a_public_class_publishes_its_namespace_manifest: {
        claim:
          'If a file declares a dollar-prefixed class, then it exports a namespace with dollar-Class, Class, and Instance for reactive classes, and no behavior is exported directly',
        impossibility:
          'a file breaking a_public_class_publishes_its_namespace_manifest passes the gate',
        red: [
          {
            files: {
              'src/Box.ts': fixture.validClass.replace(
                '  export type Instance = typeof Class.Instance;\n',
                ''
              ),
              'src/Tools.ts':
            ...
            expectFindings: [
              /lacks `export type Instance/,
              /behavioral object is exported directly/
            ],
            expectCount: 2
          }
        ],
      ...
      a_class_file_is_named_after_its_class: {
        claim:
          'If a file declares dollar-X, then the file is X.ts, or a dotted kit path whose dots removed spell X, and the namespace is X',
        impossibility: 'a file breaking a_class_file_is_named_after_its_class passes the gate',
        red: [
          {
            files: { 'src/Crate.ts': fixture.validClass },
            expectFindings: [/`Crate\.ts` declares `\$Box`/]
          }
        ],
        ...
        green: [
          {
            files: {
              ...box,
              'src/Box.Lid.ts':
                'class $BoxLid {\n  spin() {\n    return 1;\n  }\n}\n\nexport namespace BoxLid {\n  export const $Class = $BoxLid;\n  export let Class = $Class;\n}\n'
            }
          },
          {
            files: {
      ...
      a_class_file_holds_only_imports_class_namespace_and_types: {
        claim:
          'If a file is a class file, then its top level is imports, the class, and its namespace, nothing else — every type it declares is a namespace member',
        impossibility:
          'a file breaking a_class_file_holds_only_imports_class_namespace_and_types passes the gate',
        red: [
          {
            files: {
              'src/Box.ts': `${fixture.validClass}\nconst DEFAULT_WIDTH = 4;\nexport function widen(box: Box.Instance) { return box.area; }\nexport type BoxSeed = { width: number };\n`
            },
            expectFindings: [/outside the class seam/, /`BoxSeed` is a type outside the namespace/],
            expectCount: 3
          }
        ],
        green: [
      ...
      the_namespace_holds_identity_and_types_only: {
        claim:
          'If a class file has a namespace, then the namespace holds $Class, Class, and type declarations, never runtime data or behavior (which live on the class as statics)',
        impossibility:
          'a file breaking the_namespace_holds_identity_and_types_only passes the gate',
        red: [
          {
            files: {
              'src/Box.ts': fixture.validClass.replace(
            ...
            expectFindings: [
              /`DEFAULT_WIDTH` is runtime data in namespace Box/,
              /`SEEDS` is runtime data/,
              /`widen` is behavior in namespace Box/
            ],
            expectCount: 3
          }
        ],
      ...
      behavior_lives_on_the_prototype_not_in_fields: {
        claim:
          'If a class member is a function, then it is a method, never a function-valued field',
        impossibility:
          'a file breaking behavior_lives_on_the_prototype_not_in_fields passes the gate',
        red: [
          {
            files: {
              'src/Box.ts': fixture.validClass.replace(
      ...
      construction_goes_through_the_namespace_class_slot: {
        claim:
          'If an instance is created, then it is new X.Class, never new dollar-X, new X.dollar-Class, or bare reactive-wrapped construction; reactive(new X.Class() as X.Instance) is the sanctioned view',
        impossibility:
          'a file breaking construction_goes_through_the_namespace_class_slot passes the gate',
        red: [
          {
            files: {
              ...box,
      ...
      the_anchor_is_static_only_when_statics_exist: {
        claim:
          'If a class declares static members, then its anchor is Static of the raw class, and if it declares none, then its anchor is the raw class itself',
        impossibility:
          'a file breaking the_anchor_is_static_only_when_statics_exist passes the gate',
        red: [
          {
            files: {
              'src/Clock.ts': fixture.staticClass.replace(
                'export const $Class = Static($Clock);',
              ...
              'src/Box.ts': fixture.validClass
                ...
                .replace('export const $Class = $Box;', 'export const $Class = Static($Box);')
            },
            expectCount: 2
          }
        ],
      ...
      static_binds_methods_and_caches_dollar_getters_per_receiver: {
        claim:
          "If the consumer's Static transforms a class, then its static methods are bound with stable identity and its dollar getters run once per receiver class",
        impossibility:
          'a file breaking static_binds_methods_and_caches_dollar_getters_per_receiver passes the gate',
        red: [
          {
            files: box,
            options: {
              staticImplementation: (<Class>(targetClass: Class) =>
                targetClass) as CheckStandard.StaticTransform
            },
            expectFindings: [
              /does not bind static methods/,
              /does not cache a dollar getter once per receiver/
            ]
          },
          {
            files: box,
            options: { staticImplementation: null },
            expectFindings: [/could not be loaded/]
          }
        ],
        green: [{ files: box }]
      },
      a_shared_store_is_a_static_readonly_field: {
        claim:
          'If a static holds shared state, then the field is readonly, and a dependency constructed at load lives in a LazyShared cell',
        ...
        red: [
          {
            ...
            expectFindings: [
              ...
              /constructs another namespace's class at module load/
            ]
          }
        ],
        green: [
      ...
      a_derived_static_getter_is_lower_camel_case: {
        claim:
          'If a static getter derives its value from other members or classes, then its name is lowerCamel, and SCREAMING_SNAKE remains for tunable constants: literals, or other SCREAMING constants composed',
        impossibility:
          'a file breaking a_derived_static_getter_is_lower_camel_case passes the gate',
        red: [
          {
            files: {
              'src/Clock.ts': fixture.staticClass.replace(
                '  static now() {',
                '  static get SCAN_LIMIT_HOURS() {\n    return Number(this.$zone.length) * 24;\n  }\n\n  static now() {'
              )
            },
            expectFindings: [
              /derives its value — a derived getter is lowerCamel \(`scanLimitHours`\)/
            ]
          }
        ],
        green: [
      ...
      static_reads_go_through_self_not_the_base_class: {
        claim:
          'If instance code reads its own statics, then it reads this.self, never the base class name or a per-site constructor cast',
        impossibility:
          'a file breaking static_reads_go_through_self_not_the_base_class passes the gate',
        red: [
          {
            files: {
              'src/Tooltip.ts': fixture.selfClass(
                'return $Tooltip.DELAY_MS + (this.constructor as typeof $Tooltip).DELAY_MS;'
              )
            },
            expectCount: 2
          }
      ...
      mutable_state_is_a_ref_returning_getter: {
        claim:
          'If a class holds mutable state, then it is a getter returning ref or shallowRef, never a mutable plain field',
        ...
        red: [
          {
            ...
            expectFindings: [/`count` is a mutable plain field/]
          }
        ],
      ...
      a_ref_is_read_and_written_through_value: {
        claim:
          'If class code writes a Ref getter, then it writes .value, never assigns over the getter',
        ...
        red: [
          {
            ...
            expectFindings: [/assigns over a Ref getter/]
          }
        ],
      ...
      a_derivation_is_a_plain_getter_unless_computed_is_justified: {
        claim:
          'If a getter allocates a computed, then a stated reason, expensive or render-suppression or stable-handle, sits above it',
        impossibility:
          'a file breaking a_derivation_is_a_plain_getter_unless_computed_is_justified passes the gate',
        red: [
          {
            files: {
              'src/Box.ts': fixture.validClass
        ...
        green: [
          {
            files: {
              'src/Box.ts': fixture.validClass
                ...
                .replace(
                  '  grow() {',
                  '  // computed: expensive — sorts every row\n  get sortedRows() {\n    return computed(() => this.sortRows());\n  }\n\n  sortRows() {\n    return [this.area];\n  }\n\n  grow() {'
                )
            }
          }
        ]
      ...
      a_composable_is_injected_by_a_one_call_dollar_getter: {
        claim:
          'If a class uses a composable or store, then a dollar getter returns the one call, never an eager field',
        impossibility:
          'a file breaking a_composable_is_injected_by_a_one_call_dollar_getter passes the gate',
        red: [
          {
            files: {
              'src/Box.ts': fixture.validClass
            ...
            expectFindings: [/runs at construction/, /does more than one call/]
          }
        ],
        green: [
          {
      ...
      instance_types_only_unwrapping_surfaces: {
        claim:
          'If a raw collection or parameter is typed, then it uses Model, and if an unwrapping surface is typed, then it uses Instance',
        ...
        red: [
          {
            files: {
              ...
              'src/Box.vue': fixture.validSfc.replace(
                'defineExpose(box as Box.Instance);',
                'defineExpose(box as Box.Model);'
              )
            },
        ...
        green: [
          {
            files: {
              ...
              'src/Box.vue': fixture.validSfc.replace(
                'new Box.Class(props)',
                'new ((props.kit?.namespace.Class as typeof Box.Class | undefined) ?? Box.Class)(props)'
              )
            }
          },
      ...
      script_setup_is_wiring_only: {
        claim:
          'If a component has a model, then its script setup declares no parallel state, derivation, watcher, or free function',
      ...
      a_lifecycle_hook_delegates_to_one_method: {
        claim:
          'If a lifecycle hook is registered in script setup, then its whole body delegates one call to the model',
      ...
      the_state_destructure_is_total: {
        claim:
          'If a template touches a Ref, then that Ref is destructured, no plain getter or method is destructured, and no state binding shadows a prop',
        ...
        red: [
          {
            ...
            expectFindings: [
              /`area` is a plain getter/,
              /`grow` is a method/,
              /`width` shadows the prop/,
              /reaches a Ref through the instance/
            ]
          }
        ],
        green: [
          {
            files: {
              'src/Box.ts': fixture.validClass
                ...
                .replace(
                  '  get width() {',
                  '  get forwardedHeight(): Ref<number> {\n    return this.height;\n  }\n  get width() {'
                ),
              'src/Box.vue': fixture.validSfc.replace(
                'const { height } = box;',
                'const { height, forwardedHeight } = box;'
              )
            }
          },
          { files: { ...box, 'src/Box.vue': fixture.validSfc } }
      ...
      one_handler_per_event: {
        claim:
          'If a template binds DOM events on one element, then no two events share a handler: a cancel gets its own method that may delegate to the lift, so a subclass can extend either alone',
        ...
        red: [
          {
            files: {
              ...
              'src/Box.vue': fixture.validSfc.replace(
                '<button @click="box.grow()">grow</button>',
                '<button @pointerup="box.grow" @pointercancel="box.grow">grow</button>'
              )
            },
        ...
        green: [
          {
            files: {
              ...
              'src/Box.vue': fixture.validSfc.replace(
                '<button @click="box.grow()">grow</button>',
                '<button @pointerup="box.grow" @pointercancel="box.cancelGrow">grow</button>'
              )
            }
          }
        ]
      ...
      template_expressions_carry_no_logic: {
        claim:
          'If a template expression is written, then it is a named read, a method call, or a structural branch, never a comparison, ternary, negation, or built string',
        ...
        red: [
          {
            files: {
              ...
              'src/Box.vue': fixture.validSfc.replace(
                '<div v-if="height > 0">{{ box.area }}</div>',
                '<div v-if="height > 0 && box.area">{{ box.area ? \'big\' : \'small\' }}</div>\n  <span :title="`Box ${box.area}`">{{ !!height }}</span>'
              )
            },
      ...
      watch_lifetime_matches_the_instance_owner: {
        claim:
          'If a class is component-scoped, then it uses plain watch, and if it outlives components, then it uses dollar-watch with a dispose path',
        ...
        red: [
          {
            ...
            expectFindings: [
              /constructed in a component's setup but uses `this\.\$watch`/,
              /no dispose path/,
              /outlives components .* but uses plain `watch`/
            ]
          }
        ],
        green: [
      ...
      a_reactive_closure_delegates_to_one_method: {
        claim:
          'If a computed or watch callback is written, then it is one arrow delegating to one method',
      ...
      a_store_is_used_lazily_and_swapped_at_the_class_slot: {
        claim:
          'If shared state is published, then it constructs lazily behind use and is never drilled as a prop or constructor argument',
        impossibility:
          'a file breaking a_store_is_used_lazily_and_swapped_at_the_class_slot passes the gate',
        red: [
          {
            files: {
              ...
              'src/Box.vue': fixture.validSfc.replace(
                ...
                'defineProps<{ width: number; app: Box.Instance }>()'
              )
            },
            expectFindings: [
              /constructs a singleton at module load/,
              /prop `app: Box\.Instance` drills a shared model/
            ]
          }
        ],
        green: [
      ...
      keyed_state_creates_on_read_and_peeks_on_write: {
        claim:
          'If a class holds a Map of refs, then reads get-or-create, writes peek, and a release path exists',
        impossibility:
          'a file breaking keyed_state_creates_on_read_and_peeks_on_write passes the gate',
        red: [
          {
            files: {
              'src/Sheet.ts': fixture.keyedClass(
                'bumpCell(cellKey: number): void {\n    let versionRef = this.cellVersions.get(cellKey);\n    if (!versionRef) {\n      versionRef = ref(0);\n      this.cellVersions.set(cellKey, versionRef);\n    }\n    versionRef.value++;\n  }\n',
                ''
              )
            },
      ...
      a_generic_reactive_class_casts_its_constructor: {
        claim:
          'If a reactive class is generic, then Class is cast back to typeof dollar-Class and Instance applies ReactiveInstance by hand',
        impossibility:
          'a file breaking a_generic_reactive_class_casts_its_constructor passes the gate',
        red: [
          {
            files: {
              'src/Scroller.ts': fixture.genericClass(
                'export let Class = Reactive($Class);',
                'export type Instance = typeof Class.Instance;'
              )
            },
            expectFindings: [/`Class` erases <T>/, /`Instance` must carry <T>/]
          }
        ],
        green: [
          {
            files: {
              'src/Scroller.ts': fixture.genericClass(
                ...
                'export type Instance<T> = ReactiveInstance<$Scroller<T>>;'
              )
            }
          }
        ]
      ...
      cross_module_class_reads_happen_inside_bodies: {
        claim:
          "If a module reads another namespace's Class, then it does so inside a getter or method body, never at module evaluation",
        impossibility:
          'a file breaking cross_module_class_reads_happen_inside_bodies passes the gate',
        red: [
          {
            files: {
              ...box,
      ...
      declarations_use_full_descriptive_names: {
        claim:
          'If a name is declared in source or tests, then it is a domain word, never a single letter or a banned abbreviation',
        ...
        red: [
          {
            files: {
              ...
              'src/Box.test.ts': fixture.validTest.replace(
                "test('height never decreases on its own', () => {",
                "test('height never decreases on its own', (_) => {"
              )
            },
          ...
          },
          {
            ...
            files: {
              ...
              'src/Box.vue': fixture.validSfc.replace(
                '<button @click="box.grow()">grow</button>',
                '<ul><li v-for="(r, i) in box.rows" :key="i">{{ r }}</li></ul>\n  <button @click="box.grow()">grow</button>'
              )
            },
        ...
        green: [
          {
            files: {
              'src/Box.ts': fixture.validClass.replace(
                '  grow() {',
                '  offset(px: number, id: string) {\n    return `${id}:${px}`;\n  }\n\n  grow() {'
              ),
              'src/Box.test.ts': fixture.validTest
            }
          },
          {
            files: {
              ...
              'src/Box.vue': fixture.validSfc.replace(
                '<button @click="box.grow()">grow</button>',
                '<ul><li v-for="(row, index) in box.rows" :key="index">{{ row }}</li></ul>\n  <button @click="box.grow()">grow</button>'
              )
            }
          }
        ]
      ...
      class_members_are_ordered_and_spaced: {
        claim:
          'If a class is written, then statics precede the constructor, the constructor precedes getters, methods come last and are separated by blank lines',
        ...
        red: [
          {
            ...
            expectFindings: [
              /the constructor follows a getter or field/,
              /a static member follows a getter or field/,
              /`onResize` is not separated from the previous method/
            ],
            expectCount: 3
          }
        ],
      ...
      the_population_and_skip_list_are_exact: {
        claim:
          'If the gate runs, then it refuses zero files, unmatched globs, unknown check names, duplicate and stale skips, and unknown or conflicting severity overrides',
        ...
        red: [
          ...
          {
            files: box,
            options: { testGlobs: ['src/**/*.test.ts'] },
            expectThrows: /test glob matches no file/
          },
          {
            files: {
              ...box,
              'skips.json': JSON.stringify([
                { path: 'src/Box.ts', check: 'No such check', reason: 'reason' }
              ])
            },
            options: { skipListPath: 'skips.json' },
            expectThrows: /unknown check name/
          },
          {
            files: {
              ...box,
              'skips.json': JSON.stringify([
                {
                  path: 'src/Box.ts',
                  check: 'a_class_file_is_named_after_its_class',
                  reason: 'first'
                },
                {
                  path: 'src/Box.ts',
          ...
          },
          {
            ...
            options: { skipListPath: 'skips.json' },
            expectFindings: [/no longer fires on src\/Box\.ts/, /src\/Gone\.ts does not exist/]
          },
          {
            files: {
              ...box,
              'skips.json': 'src/Box.ts\tA class file is named after its class\treason\n'
            },
            options: { skipListPath: 'skips.json' },
            expectThrows: /a JSON array of \{ path, check, reason \}/
          },
          {
            files: {
              ...box,
              'skips.json': JSON.stringify([
                { path: 'src/Box.ts', check: 'a_class_file_is_named_after_its_class' }
              ])
            },
            options: { skipListPath: 'skips.json' },
            expectThrows: /entry 1: .*reason/
          },
          {
            files: box,
            options: { warnChecks: ['No such check'] },
            expectThrows: /unknown check name/
          },
          {
            files: box,
            options: {
              warnChecks: ['a_class_file_is_named_after_its_class'],
              offChecks: ['a_class_file_is_named_after_its_class']
            },
            expectThrows: /both warn and off/
          }
        ],
        ...
        green: [
          {
            files: {
              'src/Crate.ts': fixture.validClass,
              'src/Crate.test.ts': fixture.validTest,
              'skips.json': JSON.stringify(
                [
                  {
                    path: 'src/Crate.ts',
                    check: 'a_class_file_is_named_after_its_class',
                    reason: 'legacy file name kept for the public import path'
                  }
                ],
                null,
                2
          ...
          {
            files: { 'src/Crate.ts': fixture.validClass, 'src/Crate.test.ts': fixture.validTest },
            options: { warnChecks: ['a_class_file_is_named_after_its_class'] },
            expectWarnings: [/`Crate\.ts` declares `\$Box`/]
          },
          // off: the check does not run at all — no finding, no warning
          {
            files: { 'src/Crate.ts': fixture.validClass, 'src/Crate.test.ts': fixture.validTest },
            options: { offChecks: ['a_class_file_is_named_after_its_class'] }
          }
        ]
      }
    };
  ...
  static defineCheck(
    name: string,
    run: (context: CheckStandard.GateContext) => CheckStandard.Finding[]
  ): CheckStandard.StandardCheck {
    return { name, enforced: true, run };
  }

  static finding(
    check: CheckStandard.StandardCheck,
    unit: CheckStandard.SourceUnit,
    line: number,
    message: string
  ): CheckStandard.Finding {
    return { check: check.name, file: unit.relativePath, line, message };
  }

  static componentFinding(
    check: CheckStandard.StandardCheck,
    component: CheckStandard.ComponentUnit,
    line: number,
    message: string
  ...
  static collectTemplateExpressions(
    nodes: TemplateChildNode[],
    into: CheckStandard.TemplateExpression[],
    aliases: CheckStandard.ForAliases[] = []
  ): void {
    ...
    for (const node of nodes) {
      if (
        ...
        for (const branch of node.branches) {
          if (branch.condition && branch.condition.type === NodeTypes.SIMPLE_EXPRESSION)
            into.push({ code: branch.condition.content, line: branch.loc.start.line, kind: 'if' });
          this.collectTemplateExpressions(branch.children, into, aliases);
        }
      } else if (node.type === NodeTypes.FOR) {
        if (node.source.type === NodeTypes.SIMPLE_EXPRESSION)
          into.push({ code: node.source.content, line: node.loc.start.line, kind: 'for' });
        const names = [node.valueAlias, node.keyAlias, node.objectIndexAlias].flatMap((alias) =>
          alias && alias.type === NodeTypes.SIMPLE_EXPRESSION ? aliasNames(alias.content) : []
        );
  ...
  static modelConstructions(
    component: CheckStandard.ComponentUnit
  ): { variable: string; namespace: string; node: ts.Node }[] {
    ...
    for (const statement of component.script.ast.statements) {
      ...
      for (const declaration of statement.declarationList.declarations) {
        ...
        if (!initializer || !ts.isNewExpression(initializer) || !ts.isIdentifier(declaration.name))
          continue;
        ...
        if (namespace)
          constructions.push({ variable: declaration.name.text, namespace, node: declaration });
      }
    }
  ...
  static propNames(component: CheckStandard.ComponentUnit): Set<string> {
    ...
    this.forEachDescendant(component.script.ast, (node) => {
      ...
      if (argument && ts.isObjectLiteralExpression(argument)) {
        for (const property of argument.properties)
          if (property.name && ts.isIdentifier(property.name)) names.add(property.name.text);
      }
    });
    return names;
  }
  ...
  static classFileOf(unit: CheckStandard.SourceUnit): CheckStandard.ClassFile | null {
    ...
    const rawClass =
      dollarClasses.find((declaration) => declaration.name!.text === `$${stem}`) ??
      dollarClasses[0];
      ...
      for (const statement of namespace.body.statements) {
        if (ts.isVariableStatement(statement)) {
          for (const declaration of statement.declarationList.declarations) {
            ...
            if (declaration.name.text === '$Class')
              anchorInitializer = declaration.initializer ?? null;
            if (declaration.name.text === 'Class')
              classInitializer = declaration.initializer ?? null;
          }
        }
        if (ts.isTypeAliasDeclaration(statement) && statement.name.text === 'Instance')
  ...
  static classFileByNamespace(
    context: CheckStandard.GateContext,
    namespace: string
  ): CheckStandard.ClassFile | null {
    for (const unit of context.sources) {
      const classFile = this.classFileOf(unit);
  ...
  static isDeclaredMember(member: ts.ClassElement): boolean {
    return !!ts
      .getModifiers(member as ts.HasModifiers)
      ?.some((modifier) => modifier.kind === ts.SyntaxKind.DeclareKeyword);
  }

  static memberName(member: ts.ClassElement): string {
    return member.name && (ts.isIdentifier(member.name) || ts.isStringLiteral(member.name))
      ? member.name.text
      : '';
  }
  ...
  static thinModelDelegation(callback: ts.Expression | ts.Node): boolean {
    ...
    if (ts.isBlock(body)) {
      ...
      body = ts.isReturnStatement(only)
        ? only.expression
  ...
  static isRefTypeNode(type: ts.TypeNode): boolean {
    return (
      ...
      ['Ref', 'ShallowRef', 'ComputedRef', 'WritableComputedRef'].includes(type.typeName.text)
    );
  }
  ...
  static qualifiedTail(typeNode: ts.TypeNode): { namespace: string; member: string } | null {
    ...
    return ts.isIdentifier(left)
      ? { namespace: left.text, member: typeNode.typeName.right.text }
      : null;
  }
  ...
  static parseHeader(unit: CheckStandard.SourceUnit): CheckStandard.GeneratorHeader {
    ...
    header.formal = block.slice(
      generatorIndex + grammar.GENERATOR.length,
      describedIndex >= 0 ? describedIndex : undefined
    );
    header.described =
      describedIndex >= 0 ? block.slice(describedIndex + grammar.GENERATOR_DESCRIBED.length) : '';
    ...
    header.formal.split('\n').forEach((line, offset) => {
      ...
      if (subject) {
        for (const path of subject[1].split(/[\s,]+/).filter(Boolean))
          header.subjects.push({ path, line: formalStartLine + offset });
        return;
      }
      ...
      if (domain) {
        ...
        header.domainClaims.set(`${symbol} — ${claim}`, {
          symbol,
          claim,
  ...
  static parseProofs(
    unit: CheckStandard.SourceUnit,
    header: CheckStandard.GeneratorHeader
  ): CheckStandard.ProofAnnotation[] {
    const grammar = this.$grammar;
    ...
    for (let index = header.endLine; index < unit.lines.length; index++) {
      ...
      if (impossible) {
        pending.push({
          type: 'impossible',
          symbol: impossible[1].trim(),
          claim: impossible[2].trim(),
          line: index + 1,
          bound: false
        });
        continue;
  ...
  static declaredInSource(sourceText: string, symbol: string): boolean {
    ...
    return new RegExp(
      `(?:class|function|interface|type|enum|namespace|const|let|var)\\s+${escaped}\\b`
    ).test(sourceText);
  }
  ...
  static resolveSeverities(options: CheckStandard.GateOptions): Map<string, 'warn' | 'off'> {
    ...
    for (const [name, severity] of Object.entries(this.severities)) {
      if (!known.has(name))
        throw new CheckStandard.GateUsageError(
          `severities: unknown check name "${name}" — --list names every check`
        );
      if (severity !== 'error') resolved.set(name, severity);
    }
    for (const [flag, severity] of [
      ['--warn', 'warn'],
      ['--off', 'off']
    ] as const) {
      ...
      for (const name of names ?? []) {
        if (!known.has(name))
          throw new CheckStandard.GateUsageError(
            `${flag}: unknown check name "${name}" — --list names every check`
          );
        resolved.set(name, severity);
      }
    }
    for (const name of options.warnChecks ?? []) {
      if ((options.offChecks ?? []).includes(name))
        throw new CheckStandard.GateUsageError(
          `"${name}" is both warn and off — pick one severity`
        );
    }
    return resolved;
  }
  ...
  static readSkipList(cwd: string, path: string): CheckStandard.SkipRow[] {
    ...
    if (!existsSync(absolute))
      throw new CheckStandard.GateUsageError(`skip-list not found: ${path}`);
    let parsed: unknown;
    try {
      parsed = JSON.parse(readFileSync(absolute, 'utf8'));
    } catch (error) {
      throw new CheckStandard.GateUsageError(
        `skip-list ${path}: not valid JSON (${(error as Error).message}) — the skip list is a JSON array of { path, check, reason }`
      );
    }
    if (!Array.isArray(parsed))
      throw new CheckStandard.GateUsageError(
        `skip-list ${path}: the skip list is a JSON array of { path, check, reason }`
      );
    ...
    parsed.forEach((entry, index) => {
      ...
      if (typeof entry !== 'object' || entry === null)
        throw new CheckStandard.GateUsageError(
          `${label}: an entry is an object — { path, check, reason }`
        );
      ...
      for (const [field, value] of [
        ...
        if (typeof value !== 'string' || !value.trim())
          throw new CheckStandard.GateUsageError(
            `${label}: "${field}" is a non-empty string — { path, check, reason }`
          );
      }
      if (!knownNames.has(checkName as string))
        throw new CheckStandard.GateUsageError(
          `${label}: unknown check name "${checkName}" — --list names every check`
        );
      const key = `${rowPath}\u0000${checkName}`;
      if (seen.has(key))
        throw new CheckStandard.GateUsageError(
          `${label}: duplicate skip for ${rowPath} / ${checkName}`
        );
      seen.add(key);
      rows.push({
        path: (rowPath as string).trim().replaceAll('\\', '/'),
        check: (checkName as string).trim(),
  ...
  static run(options: CheckStandard.GateOptions): CheckStandard.GateResult {
    ...
    if (!options.sourceRoots.length)
      throw new CheckStandard.GateUsageError('at least one --source-root is required');
    const testMatchers = options.testGlobs.map((glob) => ({
      glob,
      regexp: this.globToRegExp(glob)
    }));
    const sources: CheckStandard.SourceUnit[] = [];
    const tests: CheckStandard.SourceUnit[] = [];
    const components: CheckStandard.ComponentUnit[] = [];
    const isTest = (relativePath: string) =>
      testMatchers.some((matcher) => matcher.regexp.test(relativePath));
    for (const root of options.sourceRoots) {
      const absoluteRoot = isAbsolute(root) ? root : resolve(cwd, root);
      if (!existsSync(absoluteRoot) || !statSync(absoluteRoot).isDirectory())
        throw new CheckStandard.GateUsageError(`source root is not a directory: ${root}`);
    ...
    for (const matcher of testMatchers) {
      ...
      for (const path of this.walk(absoluteBase)) {
        const relativePath = relative(cwd, path).replaceAll('\\', '/');
        if (matcher.regexp.test(relativePath) && !tests.some((unit) => unit.path === path))
          tests.push(this.toUnit(cwd, path));
      }
    }
    if (!sources.length)
      throw new CheckStandard.GateUsageError(
        `no source files discovered under ${options.sourceRoots.join(', ')} — refusing to pass over nothing`
      );
    for (const matcher of testMatchers) {
      if (!tests.some((unit) => matcher.regexp.test(unit.relativePath)))
        throw new CheckStandard.GateUsageError(`test glob matches no file: ${matcher.glob}`);
    }
    ...
    for (const row of skips) {
      if (!used.has(row)) {
        ...
        findings.push({
          check: this.the_population_and_skip_list_are_exact.name,
          file: options.skipListPath ?? 'skip-list',
          line: row.line,
          message
        });
      }
    }
    const byPlace = (first: CheckStandard.Finding, second: CheckStandard.Finding) =>
    ...
    return {
      ...
      off: this.checks
        .filter((entry) => severities.get(entry.name) === 'off')
  ...
  static prove(options?: { completenessOnly?: boolean; only?: string }): CheckStandard.ProveReport {
    ...
    const selected = options?.only
      ? this.checks.filter((entry) => entry.name === options.only)
      : this.checks;
    if (options?.only && !selected.length)
      problems.push(`prove: unknown check name "${options.only}" — --list names every check`);
    if (!options?.only) {
      for (const name of Object.keys(proofs)) {
        if (!this.checks.some((entry) => entry.name === name))
          problems.push(`proof without a manifest check: ${name}`);
      }
    }
    for (const check of selected) {
      const asGetter = (this as unknown as Record<string, CheckStandard.StandardCheck | undefined>)[
        check.name
      ];
      if (asGetter?.name !== check.name)
        problems.push(
          `${check.name}: the name is not its getter — one snake_case form is the whole identity (getter, finding label, skip token, severity key)`
        );
      const proof = proofs[check.name];
      if (!proof) {
        problems.push(
          `${check.name}: no constitution entry — a manifest check carries its claim, impossibility, and both proof arms`
        );
        continue;
      }
      if (!/^If .+, then .+/.test(proof.claim))
        problems.push(`${check.name}: the claim is not an if-then`);
      if (!proof.impossibility) problems.push(`${check.name}: no impossibility`);
      if (!proof.red.length)
        problems.push(`${check.name}: no red arm — a check that cannot fail proves nothing`);
      if (!proof.green.length)
        problems.push(
          `${check.name}: no green arm — silence on the conforming form is half the proof`
        );
      if (options?.completenessOnly) continue;
      for (const [kind, arms] of [
        ['red', proof.red],
        ['green', proof.green]
      ] as const) {
        for (const arm of arms) {
          ...
          try {
            writeFileSync(
              ...
              JSON.stringify(arm.manifest ?? { name: 'consumer', dependencies: { ivue: '*' } })
            );
            ...
            if (arm.options && 'staticImplementation' in arm.options)
              gateOptions.staticImplementation = arm.options.staticImplementation ?? null;
            ...
            const armTestsSeverity =
              !!arm.options && ('warnChecks' in arm.options || 'offChecks' in arm.options);
            ...
            try {
              ...
              if (!armTestsSeverity) {
                findings = [
                  ...findings,
                  ...result.warnings.filter((item) => item.check === check.name)
                ];
                warnings = [];
              }
            } catch (error) {
            ...
            if (arm.expectThrows) {
              if (!thrown || !arm.expectThrows.test(thrown.message))
                problems.push(
                  `${check.name} ${kind} arm: expected a refusal matching ${arm.expectThrows} — got ${thrown ? thrown.message : `${findings.length} finding(s)`}`
                );
            } else if (thrown) {
              ...
              for (const expected of arm.expectWarnings ?? []) {
                const pattern =
                  typeof expected === 'string'
                    ...
                    : expected;
                if (!warnings.some((item) => pattern.test(item.message)))
                  problems.push(`${check.name} ${kind} arm: no warning matches ${pattern}`);
              }
              if (!arm.expectWarnings?.length && warnings.length)
                problems.push(
                  `${check.name} ${kind} arm: unexpected warning(s): ${warnings[0].message}`
                );
              if (kind === 'red') {
                for (const expected of arm.expectFindings ?? []) {
                  const pattern =
                    typeof expected === 'string'
                      ? new RegExp(expected.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'))
                      : expected;
                  if (!findings.some((item) => pattern.test(item.message)))
                    problems.push(`${check.name} red arm: no finding matches ${pattern}`);
                }
                if (!arm.expectFindings?.length && !findings.length)
                  problems.push(`${check.name} red arm: the planted defect produced no finding`);
                if (arm.expectCount !== undefined && findings.length !== arm.expectCount)
                  problems.push(
                    `${check.name} red arm: expected ${arm.expectCount} finding(s), got ${findings.length}`
                  );
              } else if (findings.length) {
                problems.push(
                  `${check.name} green arm: the conforming form produced ${findings.length} finding(s): ${findings[0].message}`
                );
              }
            }
...
name, duplicate or stale skip row). Paths in findings are relative to the cwd.`;
    ...
    for (let index = 0; index < argv.length; index++) {
      ...
      try {
        if (argument === '--help' || argument === '-h') {
          ...
          if (argv.length > 1)
            throw new CheckStandard.GateUsageError('--list takes no other arguments');
          for (const entry of this.checks) {
          ...
          if (sourceRoots.length || testGlobs.length || skipListPath || index + 1 < argv.length)
            throw new CheckStandard.GateUsageError(
              '--prove runs the constitution over its own fixture checkouts — it does not combine with --source-root, --test-glob, or --skip-list'
            );
          const report = this.prove(only ? { only } : undefined);
          for (const problem of report.problems) console.error(problem);
          console.log(
            `ivue-standards-check --prove${only ? ` "${only}"` : ''}: ${report.ran.red} red arm(s), ${report.ran.green} green arm(s), ${report.problems.length} problem(s)`
          );
    ...
    for (const item of result.findings)
      console.error(`${item.file}:${item.line}: [${item.check}] ${item.message}`);
    for (const item of result.warnings)
      console.error(`warn: ${item.file}:${item.line}: [${item.check}] ${item.message}`);
    console.log(
    ...
    if (result.off.length)
      console.log(`off by config (${result.off.length}): ${result.off.join(' · ')}`);
    if (result.unenforced.length)
      console.log(
        `not enforced yet (${result.unenforced.length}): ${result.unenforced.join(' · ')}`
      );
...
export namespace CheckStandard {
  ...
  export type StaticTransform = <Class extends new (...arguments_: any[]) => any>(
    targetClass: Class
  ) => Class;
