 ...
 *
 * A factory that, for EVERY cell, eagerly creates a `ref` for the raw value and
 * an eager `computed()` for each derived value (display, isNumber, cssClass and
 * the hot numericValue). This is the textbook Vue 3 way to give an entity
 * fine-grained reactivity — and every one of the 100,000 cells pays the full
...
export interface ComposableCell {
  ...
  /** Read every derived value once (used only by the measurement harness). */
  touch(): void;
}
