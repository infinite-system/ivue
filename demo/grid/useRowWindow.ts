/**
 * Shared, identical row-windowing for BOTH arms.
 *
 * Given a (reactive) row count, a fixed row height and a viewport height, it
 * derives which contiguous band of rows is on screen plus the translate offset
 * and total scroll height. This is the ENTIRE virtualization strategy and it is
 * byte-for-byte the same code for the composable arm and the ivue arm — only
 * the per-cell model differs between them.
 */
