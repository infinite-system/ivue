...
import { useReactiveItem } from './useReactiveItem';

const numItems = ref(1000000);
const items = ref<ReturnType<typeof useReactiveItem>[]>([]);
const initTime = ref(0);

const extend = (self: any) => {
  return {
    area: computed(() => 'NEW AREA: ' + (self.area.value)),
    update: () => {
      const oldUpdate = self.update();
      self.width.value = oldUpdate * 10000;
    },
  };
};
...
const createItems = () => {
  ...
  for (let i = 0; i < numItems.value; i++) {
    ...
    newItems.push(useReactiveItem({ id: i }));
  }
