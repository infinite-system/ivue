...
const props = defineProps(NewsletterSignup.Class.props);

const signup = new NewsletterSignup.Class(props as NewsletterSignup.Props);
  ...
  <Transition name="newsletter-slide">
    ...
    >
      ...
      <p class="newsletter__pitch">
        New patterns, releases, and measured numbers from the JavaScript frontier — the whole blog
        in {{ $archiveDays }}
        days, one post every other day, at your local morning.
      </p>
      <p v-if="signup.quickLeadVisible" class="newsletter__quick-lead">
      ...
      <p v-else-if="message" class="newsletter__message" :class="{ error: signup.failed }">
        {{ message }}
      </p>
    </div>
  </Transition>
