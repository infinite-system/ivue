...
**Scope:** `VirtualScroller.ts`: the `scroll` and `selection` props, `scrollKnobs`, `selectionKnobs`, the constructor's `nestedProps` seam, `lenisMotion`, `tuneMotion`, `autoscrollProfiles`; `lenis.ts` `tune`, `wheelMaxPxPerMs`, `touchMaxPxPerMs`, `lastInputTouch`; `animate.ts` `maxPxPerMs`; `virtual-scroll.ts` `tune`; `ivue/extras` `nestedProps`.
