...
<template>
  <div class="pane pane-wide">
    <p class="note">
      Every cell in columns E–J (and every odd column beyond) holds a real formula — evaluated by
      fast-formula-parser, with the dependency graph discovered by Vue. Click a cell to edit; watch
      dependents cascade.
    </p>
    <div class="row" style="margin-bottom: 12px">
      <button class="btn primary" type="button" :disabled="loading" @click="grid.createSmall()">
        {{ grid.createLabel }}
      </button>
      <button
        class="btn"
      ...
      <span v-if="grid.hasModel" class="mono">
        {{ grid.modelCellsLabel }} cells · created in {{ grid.creationLabel }} ms
      </span>
    </div>
