...
<template>
  <div ref="rootElement" class="ac-picker" :class="model.menuClass">
    <button
      ...
      class="ac-model"
      :aria-expanded="open"
      :title="model.composer.modelHint"
      @click="model.toggle()"
      @keydown="model.onKeydown($event)"
    >
      <span class="ac-model-dot" aria-hidden="true"></span>
      <svg class="ac-model-icon" viewBox="0 0 24 24" aria-hidden="true">
    ...
    <div v-if="open" class="ac-picker-menu" role="listbox">
      ...
      <button
        v-for="option in model.models"
        :key="option.id"
        type="button"
        class="ac-picker-card"
        :class="model.cardClass(option)"
        role="option"
        :aria-selected="model.isPicked(option)"
        @click="model.pick(option)"
      >
        <svg class="ac-picker-icon" viewBox="0 0 24 24" aria-hidden="true">
          <path :d="model.iconFor(option)" />
        </svg>
        <span class="ac-picker-body">
          ...
          <span class="ac-picker-meter" :title="model.speedLabel(option)">
            <span class="ac-picker-track"
              ><span class="ac-picker-bar ac-picker-speed" :style="model.speedStyle(option)"></span
            ></span>
          ...
          <span class="ac-picker-meter" :title="model.firstTokenLabel(option)">
            <span class="ac-picker-track"
              ><span class="ac-picker-bar ac-picker-wait" :style="model.waitStyle(option)"></span
            ></span>
