...
class $EditCall extends ToolCallModel.$Class {
  ...
  get hunks(): EditCall.Hunk[] {
    ...
    return Array.isArray(patch) ? (patch as EditCall.Hunk[]).filter((hunk) => Array.isArray(hunk.lines)) : [];
  }

  /** the unified diff: hunks from the record, or a plain remove-then-add from the strings */
  get diff(): string {
    if (this.hunks.length)
      return this.hunks.map((hunk) => `@@ -${hunk.oldStart},${hunk.oldLines} +${hunk.newStart},${hunk.newLines} @@\n${hunk.lines.join('\n')}`).join('\n');
  ...
  override get sections(): ToolCallModel.Section[] {
    const sections: ToolCallModel.Section[] = [{ title: this.filePath, code: this.diff, lang: 'diff' }];
