...
const assistant = (uuid: string, apiId: string, block: object, seconds: number, extra: object = {}) =>
  line({ type: 'assistant', uuid, parentUuid: null, timestamp: at(seconds), message: { id: apiId, model: 'claude-opus-5', content: [block], usage: { output_tokens: 7 } }, ...extra });
const toolResult = (uuid: string, toolUseId: string, content: unknown, seconds: number, structured: unknown = null, extra: object = {}) =>
  line({ type: 'user', uuid, timestamp: at(seconds), message: { role: 'user', content: [{ type: 'tool_result', tool_use_id: toolUseId, content }] }, toolUseResult: structured, ...extra });
const prompt = (uuid: string, text: string, seconds: number, extra: object = {}) =>
  line({ type: 'user', uuid, timestamp: at(seconds), message: { role: 'user', content: text }, ...extra });
...
const SESSION = [
  ...
  assistant('a3', 'api-1', { type: 'tool_use', id: 'call-1', name: 'Bash', input: { command: 'npm test\necho done' } }, 1),
  toolResult('r1', 'call-1', 'ok', 4, { stdout: 'ok', stderr: '', interrupted: false }),
  assistant('a4', 'api-2', { type: 'tool_use', id: 'call-2', name: 'Read', input: { file_path: '/home/parallels/dev/app.ts' } }, 5),
  toolResult('r2', 'call-2', '1\tconst a = 1;', 6, { type: 'text', file: { filePath: '/home/parallels/dev/app.ts', content: 'const a = 1;', numLines: 1 } }),
  assistant('a5', 'api-3', { type: 'tool_use', id: 'call-3', name: 'Edit', input: { file_path: '/home/parallels/dev/app.ts', old_string: 'a', new_string: 'b' } }, 7),
  toolResult('r3', 'call-3', 'edited', 8, { filePath: '/home/parallels/dev/app.ts', structuredPatch: [] }),
  line({ type: 'system', uuid: 'sys1', subtype: 'turn_duration', durationMs: 8000, timestamp: at(8) }),
  prompt('u2', 'now the agent', 9),
  assistant('a6', 'api-4', { type: 'tool_use', id: 'call-4', name: 'Agent', input: { description: 'survey', prompt: 'look around' } }, 10),
  prompt('s1', 'look around', 10, { isSidechain: true, parentUuid: null }),
  assistant('s2', 'api-s1', { type: 'text', text: 'found it' }, 11, { isSidechain: true, parentUuid: 's1' }),
  toolResult('r4', 'call-4', [{ type: 'text', text: 'Findings.' }], 12, { agentId: 'agent-1', status: 'done' }),
  assistant('a7', 'api-5', { type: 'text', text: 'Done, mailed ekalashnikov@gmail.com' }, 13),
  line({ type: 'user', uuid: 'c1', timestamp: at(14), isCompactSummary: true, message: { role: 'user', content: 'Summary of everything' } }),
  prompt('m1', 'meta text', 15, { isMeta: true }),
  assistant('a8', 'api-6', { type: 'tool_use', id: 'call-5', name: 'Write', input: { file_path: 'x', content: 'y' } }, 16),
...
describe('SessionLog', () => {
  ...
  it('merges split turns, joins results, contracts runs to batches and keeps a single call bare', () => {
    ...
    expect(messages.map((message) => message.role)).toEqual(['user', 'assistant', 'assistant', 'system', 'user', 'assistant', 'assistant', 'system', 'assistant']);
    ...
    expect(duration.parts[0]).toMatchObject({ kind: 'system', subtype: 'turn_duration', text: 'Turn took 8.0s', durationMs: 8000 });
    ...
    expect(compaction.parts[0]).toMatchObject({ kind: 'system', subtype: 'compaction', text: 'Context compacted', detail: 'Summary of everything' });
    expect(single.parts[0].kind).toBe('tool_call');
    expect(SessionLog.Class.messageText(batch)).toBe('2 tool calls: Read, Edit');
    expect(SessionLog.Class.messageText(turn)).toBe('thinking · 4 chars\nOn it.\ntool: Bash npm test');
  });
  ...
  it('drops bookkeeping, meta prompts and bad lines, and scrubs when asked', () => {
    ...
    for (const message of messages)
      for (const part of message.parts) if (part.kind === 'tool_batch') expect(part.calls.length).toBeGreaterThanOrEqual(2);
    const parser = new SessionParser.Class({ scrub: true, systemSubtypes: ['compaction'] });
    for (const entry of SESSION.split('\n')) parser.line(entry);
    expect(parser.finish().filter((message) => message.role === 'system').map((message) => message.parts[0])).toMatchObject([{ subtype: 'compaction' }]);
  ...
  it('contracts consecutive calls inside one mixed turn and leaves a lone call alone', () => {
    const mixed = [
      ...
      assistant('b2', 'api-9', { type: 'tool_use', id: 'c1', name: 'Grep', input: { pattern: 'foo' } }, 0),
      assistant('b3', 'api-9', { type: 'tool_use', id: 'c2', name: 'Glob', input: { pattern: '*.ts' } }, 0),
      assistant('b4', 'api-9', { type: 'text', text: 'and one more' }, 0),
      assistant('b5', 'api-9', { type: 'tool_use', id: 'c3', name: 'WebFetch', input: { url: 'https://ivue.dev' } }, 0),
    ].join('\n');
    const [message] = SessionLog.Class.parse(mixed);
    expect(message.parts.map((part) => part.kind)).toEqual(['text', 'tool_batch', 'text', 'tool_call']);
  ...
  it('formats durations, epochs, result text and images', () => {
    ...
    expect(SessionLog.Class.resultText([{ type: 'text', text: 'a' }, { type: 'image', source: { media_type: 'image/png', data: 'AAAA' } }])).toBe('a\n[image]');
    expect(SessionLog.Class.resultImages([{ type: 'image', source: { media_type: 'image/png', data: 'AAAA' } }])).toEqual(['data:image/png;base64,AAAA']);
    expect(SessionLog.Class.resultImages('plain')).toEqual([]);
    expect(SessionLog.Class.callSummary({ id: 'x', name: 'Mystery', input: { anything: 'first\nsecond' }, state: 'pending', result: null, durationMs: null, startedAt: 0, children: null })).toBe('first');
    expect(SessionLog.Class.callSummary({ id: 'x', name: 'Mystery', input: { count: 2 }, state: 'pending', result: null, durationMs: null, startedAt: 0, children: null })).toBe('');
    expect(SessionLog.Class.partText({ kind: 'attachment', name: 'a.png', size: 1, mimeType: 'image/png', url: 'blob:x' })).toBe('a.png');
  });
  ...
  it('a sidechain with no Agent call becomes a subagent note at the end', () => {
    const orphan = [
      ...
      assistant('s2', 'api-s', { type: 'text', text: 'side answer' }, 2, { isSidechain: true, parentUuid: 's1' }),
    ].join('\n');
    const messages = SessionLog.Class.parse(orphan);
    expect(messages).toHaveLength(2);
    expect(messages[1].parts[0]).toMatchObject({ kind: 'system', subtype: 'subagent', text: 'Subagent thread (2 messages)' });
