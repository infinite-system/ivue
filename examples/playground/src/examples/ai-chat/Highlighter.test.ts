...
describe('Highlighter', () => {
  ...
  it('renders plain escaped code at once, and stays plain when shiki is unavailable or the code is huge', async () => {
    expect(Highlighter.Class.plain('a < b')).toBe(
      '<pre class="chat-code"><code>a &lt; b</code></pre>'
    );
    vi.spyOn(Highlighter.Class, 'load').mockResolvedValue(null);
    expect(await Highlighter.Class.highlight('const a = 1;', 'ts')).toBe(
      '<pre class="chat-code"><code>const a = 1;</code></pre>'
    );
    expect(await Highlighter.Class.highlight('x', 'text')).toBe(
      '<pre class="chat-code"><code>x</code></pre>'
    );
    expect(
      await Highlighter.Class.highlight('x'.repeat(Highlighter.Class.MAXIMUM_CHARS + 1), 'ts')
    ).toContain('<pre class="chat-code">');
  });
  ...
  it('uses the engine when it loads, caches by code and language, and falls back when it throws', async () => {
    const codeToHtml = vi.fn(
      (code: string, options: { lang: string }) =>
        `<pre class="shiki" style="x"><code><span class="line">${code} ${options.lang}</span></code></pre>`
    );
    ...
    expect(first).toBe(
      '<pre class="chat-code shiki" style="x"><code><span class="line">let x typescript</span></code></pre>'
    );
    ...
    expect(await Fresh.highlight('other', 'json')).toBe(
      '<pre class="chat-code"><code>other</code></pre>'
    );
  });
});
