...
describe('Markdown', () => {
  ...
  it('keeps a fence language on the block, closes an open fence, and embeds media lines', () => {
    expect(Markdown.Class.render('```ts\nconst a = 1;\n```')).toBe('<pre class="chat-code" data-lang="ts"><code>const a = 1;</code></pre>');
    expect(Markdown.Class.render('```\nopen')).toBe('<pre class="chat-code" data-lang=""><code>open</code></pre>');
    const media = Markdown.Class.render('Watch:\n\nhttps://youtu.be/dQw4w9WgXcQ\n\nhttps://cdn.test/clip.mp4');
    expect(media).toContain('youtube-nocookie.com/embed/dQw4w9WgXcQ');
    expect(media).toContain('<video class="chat-video" controls preload="metadata" src="https://cdn.test/clip.mp4">');
