...
class $Scrub {
  ...
  static readonly RULES: Scrub.Rule[] = [
    { name: 'email', pattern: /[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/g, replacement: 'user@example.com' },
    { name: 'authorization-header', pattern: /(authorization\s*[:=]\s*)["']?[^"'\n,}]+/gi, replacement: '$1[redacted]' },
    { name: 'bearer', pattern: /\b(Bearer|Basic)\s+[A-Za-z0-9._~+/=-]{8,}/g, replacement: '$1 [redacted]' },
    { name: 'api-key', pattern: /\b(?:sk-(?:ant-)?[A-Za-z0-9_-]{20,}|(?:sk|pk|rk)_(?:live|test)_[A-Za-z0-9]{16,}|gh[pousr]_[A-Za-z0-9]{30,}|xox[abp]-[A-Za-z0-9-]{20,}|AKIA[0-9A-Z]{16})\b/g, replacement: '[redacted-key]' },
    { name: 'secret-assignment', pattern: /\b([A-Z][A-Z0-9_]*(?:SECRET|TOKEN|PASSWORD|API_KEY|PRIVATE_KEY)[A-Z0-9_]*)(\s*[:=]\s*["']?)(?!e2e-local-secret\b|process\.|env\.|import\.|\$)[A-Za-z0-9._~+/=-]{12,}/g, replacement: '$1$2[redacted]' },
    { name: 'pem-block', pattern: /-----BEGIN [^\n]*?-----[\s\S]*?(?:-----END [^\n]*?-----|$)/g, replacement: '[redacted PEM block]' },
    ...
    { name: 'lan-ip', pattern: /\b(?:10|192\.168|172\.(?:1[6-9]|2\d|3[01]))(?:\.\d{1,3}){2,3}\b/g, replacement: '10.0.0.1' },
    { name: 'session-url', pattern: /https:\/\/claude\.ai\/code\/session_[A-Za-z0-9]+/g, replacement: 'https://claude.ai/code/session_[redacted]' },
  ];
  ...
  static text(input: string, counts?: Scrub.Counts): string {
    ...
    for (const rule of this.RULES) {
      output = output.replace(rule.pattern, (...match: unknown[]) => {
        ...
        return rule.replacement.replace(/\$(\d)/g, (whole, group: string) => String(match[Number(group)] ?? ''));
      });
    }
    return output;
  }
  ...
  static value<Value>(input: Value, counts?: Scrub.Counts): Value {
    ...
    if (Array.isArray(input)) return input.map((entry) => this.value(entry, counts)) as unknown as Value;
    if (input && typeof input === 'object') {
      const output: Record<string, unknown> = {};
      for (const [key, entry] of Object.entries(input as Record<string, unknown>)) output[key] = this.value(entry, counts);
