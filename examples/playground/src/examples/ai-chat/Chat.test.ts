...
describe('Chat', () => {
  ...
  it('expansion, focus, the bottom test, and the jump chip', async () => {
    ...
    chat.toggleIndex();
    expect(chat.indexOpen).toBe(true);
    expect(chat.sidebarTab.value).toBe('Index');
    chat.closeIndex();
    expect(chat.indexOpen).toBe(false);
    chat.search('Scrub.ts');
    expect(chat.sidebarTab.value).toBe('Index');
    expect(chat.searchRequest.value).toBe('Scrub.ts');
    chat.toggleSidebar('Files');
    expect(chat.sidebarTab.value).toBe('Files');
    chat.toggleSidebar('Files');
  ...
  it('content growing under a reader at the bottom keeps the pin without moving them; a scroll up lets go', () => {
    ...
    chat.pinToBottom();
    expect(scroller.setScrollPosition).toHaveBeenLastCalledWith(-800);
