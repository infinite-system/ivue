...
<template>
  <form class="ac-composer" :class="{ 'ac-drag-over': dragOver, 'ac-streaming': chat.isStreaming }" @submit.prevent="model.send()" @dragover="model.onDragOver($event)" @dragleave="model.onDragLeave()" @drop="model.onDrop($event)">
    <div class="ac-composer-card">
      ...
      <textarea ref="inputElement" v-model="draft" class="ac-input" rows="1" :placeholder="model.placeholder" @keydown="model.onKeydown($event)" @paste="model.onPaste($event)"></textarea>
