...
// domain-invariant: $PanelFiles — If a loaded message's call names a file, then the file is listed with its touches counted by kind; an unloaded row contributes nothing
// domain-invariant: $PanelFiles — If a file is open, then its records follow it in the list, each a diff of its call; closed, the list holds files only
Impossible if true: a file listed from a row whose page has not landed

=== GENERATOR-DESCRIBED ===
$PanelFiles walks the loaded rows' messages and their folded children for Read, Edit, NotebookEdit and Write calls, keyed by path.
*/
...
describe('PanelFiles', () => {
  // domain-invariant: $PanelFiles — If a loaded message's call names a file, then the file is listed with its touches counted by kind; an unloaded row contributes nothing
  // impossible-if-true: $PanelFiles — a file listed from a row whose page has not landed
  ...
  it('counts reads, edits and writes per file across calls, batches and folded threads, most touched first, and opens a file as a search', () => {
    ...
    const panel = new PanelFiles.Class({ chat });
    expect(
  ...
  it('opens a file into its records as diffs in one flat list, folds it back, and a record jumps the thread', () => {
    ...
    expect(panel.isRecordOpen(editRow.record as PanelFiles.Record)).toBe(false);
    expect(panel.diffSummary(editRow.record as PanelFiles.Record)).toBe('+2 −2');
    expect(panel.diffSummary(writeRow.record as PanelFiles.Record)).toBe('+30');
    expect(panel.diffSummary(readRow.record as PanelFiles.Record)).toBe(
      'read from line 10, 5 lines'
    );
    panel.toggleRecord(editRow.record as PanelFiles.Record);
    expect(panel.isRecordOpen(editRow.record as PanelFiles.Record)).toBe(true);
    expect(panel.recordClass(editRow.record as PanelFiles.Record)['ac-open']).toBe(true);
    panel.toggleRecord(editRow.record as PanelFiles.Record);
    expect(panel.isRecordOpen(editRow.record as PanelFiles.Record)).toBe(false);
    expect(panel.diffOf(editRow.record as PanelFiles.Record)).toEqual([
      { sign: '-', text: 'a' },
      { sign: '-', text: 'b' },
      { sign: '+', text: 'a' },
      { sign: '+', text: 'c' }
    ]);
    const writeDiff = panel.diffOf(writeRow.record as PanelFiles.Record);
    expect(writeDiff).toHaveLength(PanelFiles.$Class.DIFF_CAP + 1);
    expect(writeDiff[0]).toEqual({ sign: '+', text: 'line 0' });
    expect(writeDiff[PanelFiles.$Class.DIFF_CAP]).toEqual({ sign: ' ', text: '… 6 more lines' });
    expect(panel.diffOf(readRow.record as PanelFiles.Record)).toEqual([
      { sign: ' ', text: 'read from line 10, 5 lines' }
    ]);
    expect(panel.recordIndexLabel(editRow.record as PanelFiles.Record)).toBe('#1');
    expect(panel.rowText(editRow)).toBe('Edit #1\n-a\n-b\n+a\n+c');
    expect(panel.rowText(panel.rows.value[0])).toBe('/x/Chat.ts');
    panel.jump(editRow.record as PanelFiles.Record);
