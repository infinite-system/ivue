...
// domain-invariant: $SidebarFiles — If a loaded message's call names a file, then the file is listed with its touches counted by kind; an unloaded row contributes nothing
// domain-invariant: $SidebarFiles — If a file is open, then its records follow it in the list, each a diff of its call; closed, the list holds files only
Impossible if true: a file listed from a row whose page has not landed

=== GENERATOR-DESCRIBED ===
$SidebarFiles walks the loaded rows' messages and their folded children for Read, Edit, NotebookEdit and Write calls, keyed by path.
*/
...
describe('PanelFiles', () => {
  // domain-invariant: $SidebarFiles — If a loaded message's call names a file, then the file is listed with its touches counted by kind; an unloaded row contributes nothing
  // impossible-if-true: $SidebarFiles — a file listed from a row whose page has not landed
  ...
  it('counts reads, edits and writes per file across calls, batches and folded threads, most touched first, and opens a file as a search', () => {
    ...
    chat.applyIndex([
      { id: 'a', speaker: 'assistant', text: 'one', calls: 1, at: 1 },
      { id: 'b', speaker: 'assistant', text: 'two', calls: 3, at: 2 },
      { id: 'c', speaker: 'assistant', text: 'unloaded', calls: 2, at: 3 }
    ]);
    ...
    const panel = new SidebarFiles.Class({ chat });
    expect(
  ...
  it('opens a file into its records as diffs in one flat list, folds it back, and a record jumps the thread', () => {
    ...
    const host = hosted(() => new Chat.Class());
    const chat = host.instance;
    chat.applyIndex([{ id: 'a', speaker: 'assistant', text: 'one', calls: 3, at: 1 }]);
    const edit = {
    ...
    expect(panel.isRecordOpen(editRow.record as SidebarFiles.Record)).toBe(false);
    expect(panel.diffSummary(editRow.record as SidebarFiles.Record)).toBe('+2 −2');
    expect(panel.diffSummary(writeRow.record as SidebarFiles.Record)).toBe('+30');
    expect(panel.diffSummary(readRow.record as SidebarFiles.Record)).toBe(
      'read from line 10, 5 lines'
    );
    panel.toggleRecord(editRow.record as SidebarFiles.Record);
    expect(panel.isRecordOpen(editRow.record as SidebarFiles.Record)).toBe(true);
    expect(panel.recordClass(editRow.record as SidebarFiles.Record)['ac-open']).toBe(true);
    panel.toggleRecord(editRow.record as SidebarFiles.Record);
    expect(panel.isRecordOpen(editRow.record as SidebarFiles.Record)).toBe(false);
    expect(panel.diffOf(editRow.record as SidebarFiles.Record)).toEqual([
      { sign: '-', text: 'a' },
      { sign: '-', text: 'b' },
      { sign: '+', text: 'a' },
      { sign: '+', text: 'c' }
    ]);
    const writeDiff = panel.diffOf(writeRow.record as SidebarFiles.Record);
    expect(writeDiff).toHaveLength(SidebarFiles.$Class.DIFF_CAP + 1);
    expect(writeDiff[0]).toEqual({ sign: '+', text: 'line 0' });
    expect(writeDiff[SidebarFiles.$Class.DIFF_CAP]).toEqual({ sign: ' ', text: '… 6 more lines' });
    expect(panel.diffOf(readRow.record as SidebarFiles.Record)).toEqual([
      { sign: ' ', text: 'read from line 10, 5 lines' }
    ]);
    expect(panel.recordIndexLabel(editRow.record as SidebarFiles.Record)).toBe('#1');
    expect(panel.rowText(editRow)).toBe('Edit #1\n-a\n-b\n+a\n+c');
    expect(panel.rowText(panel.rows.value[0])).toBe('/x/Chat.ts');
    panel.jump(editRow.record as SidebarFiles.Record);
