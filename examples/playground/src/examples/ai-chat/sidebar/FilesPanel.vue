  ...
  <section class="ac-panel-pane ac-files">
    ...
    <div class="ac-index-filters ac-files-filters">
      <div class="ac-seg" role="group" aria-label="Touched by">
        <button v-for="option in model.kindOptions" :key="option.value" type="button" :class="{ 'ac-on': model.isKind(option.value) }" @click="model.setKind(option.value)">{{ option.label }}</button>
      </div>
    </div>
      ...
      <component :is="model.kit.Scroller.vue" ref="scroller" :kit="model.kit.Scroller" scrollbar :auto-repeat="false" :model-value="rows" :assumed-size="48" :padding-quantity="8" :selection-text="model.rowText">
        <template #item="{ item }">
          <div v-if="model.isFile(item)" class="ac-file" :class="model.fileClass(item)" :title="item.file.path" @click="model.toggle(item.file)">
            ...
            <button type="button" class="ac-file-go ac-go" title="Open the index on this file" @click.stop="model.open(item.file)"><svg class="ac-go-icon" viewBox="0 0 24 24" aria-hidden="true"><path :d="model.openIcon" /></svg></button>
          </div>
          <div v-else class="ac-file-record" :class="model.recordClass(item.record)" @click="model.toggleRecord(item.record)">
            <div class="ac-file-record-head">
              ...
              <button type="button" class="ac-file-record-go ac-go" title="Show this message in the chat" @click.stop="model.jump(item.record)"><svg class="ac-go-icon" viewBox="0 0 24 24" aria-hidden="true"><path :d="model.jumpIcon" /></svg></button>
            </div>
