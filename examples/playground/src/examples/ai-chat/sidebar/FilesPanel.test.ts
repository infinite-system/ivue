...
describe('FilesPanel', () => {
  ...
  it('opens a file into its records as diffs in one flat list, folds it back, and a record jumps the thread', () => {
    ...
    const edit = { ...call('1', 'Edit', '/x/Chat.ts'), input: { file_path: '/x/Chat.ts', old_string: 'a\nb', new_string: 'a\nc' } };
    const write = { ...call('2', 'Write', '/x/Chat.ts'), input: { file_path: '/x/Chat.ts', content: Array.from({ length: 30 }, (_, at) => `line ${at}`).join('\n') } };
    const read = { ...call('3', 'Read', '/x/Chat.ts'), input: { file_path: '/x/Chat.ts', offset: 10, limit: 5 } };
    chat.rows.value[0].message = message('a', 0, [{ kind: 'tool_batch', calls: [edit, write, read], messageIds: ['a'] }]);
    const panel = new FilesPanel.Class({ chat });
    ...
    expect(panel.diffOf(readRow.record as FilesPanel.Record)).toEqual([{ sign: ' ', text: 'read from line 10, 5 lines' }]);
