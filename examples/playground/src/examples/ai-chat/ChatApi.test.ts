...
const call = (id: string, name: string): SessionLog.ToolCall => ({ id, name, input: { command: 'ls' }, state: 'done', result: { text: 'ok', images: [], isError: false, structured: null }, durationMs: 1200, startedAt: 0, children: null });

describe('ChatApi', () => {
  beforeEach(() => {
    ChatApi.Class.configure({ baseUrl: '/sample/', requestCount: 0, seed: 7, simulateLatency: false });
  });
  afterEach(() => {
    vi.restoreAllMocks();
    vi.useRealTimers();
  ...
  it('fetches the sample by name and logs bytes and time; a bad status throws', async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation(async (url) => new Response(String(url).endsWith('meta.json') ? body : 'nope', { status: String(url).endsWith('meta.json') ? 200 : 404 }));
  ...
  it('the latency model is deterministic and off when asked', () => {
    ...
    for (const wait of first.slice(0, 10)) expect(wait).toBeGreaterThanOrEqual(ChatApi.Class.LATENCY_MS[0]);
  ...
  it('replays a turn as ordered events at the model pace, and aborts cleanly', async () => {
    ...
    const source: SessionLog.Message = {
      id: 'm', index: 0, role: 'assistant', timestamp: 0, sidechain: false,
      parts: [
    ...
    const events: string[] = [];
    const run = (async () => {
      for await (const event of ChatApi.Class.stream(source, model)) events.push('call' in event ? `${event.type}:${event.call.id}` : `${event.type}:${event.text}`);
    })();
    await vi.advanceTimersByTimeAsync(60_000);
    await run;
    expect(events).toEqual([
      'thinking_start:', 'thinking_token:plan ', 'thinking_token:it', 'thinking_end:',
      'token:Two ', 'token:words',
      'tool_call:c1', 'tool_result:c1', 'tool_call:c2', 'tool_result:c2', 'tool_call:c3', 'tool_result:c3',
      'done:',
    ]);
    ...
    const abortedRun = (async () => {
      try {
        for await (const event of ChatApi.Class.stream(source, model, controller.signal)) aborted.push(event.type);
      } catch (error) {
  ...
  it('an upload makes an object URL and reads image size locally, with no fetch', async () => {
    ...
    expect(image).toEqual({ kind: 'attachment', name: 'shot.png', size: 1, mimeType: 'image/png', url: 'blob:local', width: 640, height: 480 });
    const file = await ChatApi.Class.upload(new File(['abc'], 'notes.txt', { type: 'text/plain' }));
    expect(file).toEqual({ kind: 'attachment', name: 'notes.txt', size: 3, mimeType: 'text/plain', url: 'blob:local' });
