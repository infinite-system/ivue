...
// domain-invariant: $ToolCallCodeBlock — If the copy button is pressed, then the whole code as given reaches the clipboard, not the capped view, and the button says so for a moment
Impossible if true: a copy of the capped view

=== GENERATOR-DESCRIBED ===
$ToolCallCodeBlock writes through the async clipboard where it exists and the legacy command where it does not.
*/
import { expect, it, vi } from 'vitest';
import { ToolCallCodeBlock } from './ToolCall.CodeBlock';
import { hosted } from '../../../../virtual-scroller/hosted';
// domain-invariant: $ToolCallCodeBlock — If the copy button is pressed, then the whole code as given reaches the clipboard, not the capped view, and the button says so for a moment
// impossible-if-true: $ToolCallCodeBlock — a copy of the capped view
it('a code block copies its whole code and says so for a moment', async () => {
  ...
  const block = hosted(() => new ToolCallCodeBlock.Class({ code: 'one\ntwo\nthree', cap: 1 }));
  ...
  vi.advanceTimersByTime(ToolCallCodeBlock.$Class.COPIED_MS);
