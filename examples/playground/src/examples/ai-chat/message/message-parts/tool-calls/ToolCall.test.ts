...
// domain-invariant: $ToolCall — If a call is expanded, then the card shows the full input and the full result, capped only by a cap the reader can lift
Impossible if true: an expanded card holds back part of its result behind anything but the cap the reader can lift

=== GENERATOR-DESCRIBED ===
$ToolCall and its tool classes read each call's own shape: shell, edit, read, write, agent, MCP, and the generic fallback.
*/
import { describe, expect, it, vi } from 'vitest';
import { Chat } from '../../../Chat';
import type { SessionLog } from '../../../SessionLog';
import { ToolCall } from './ToolCall';
import { ToolCallBash } from './ToolCall.Bash';
import { ToolCallEdit } from './ToolCall.Edit';
import { ToolCallRead } from './ToolCall.Read';
import { ToolCallWrite } from './ToolCall.Write';
import { ToolCallAgent } from './ToolCall.Agent';
...
function makeCall(
  ...
  result: Partial<SessionLog.ToolResult> | null = {
    text: '',
    images: [],
    isError: false,
    structured: null
  },
  extra: Partial<SessionLog.ToolCall> = {}
): SessionLog.ToolCall {
  return {
    id: `${name}-1`,
    name,
    input,
    state: result ? (result.isError ? 'failed' : 'done') : 'pending',
    result: result ? { text: '', images: [], isError: false, structured: null, ...result } : null,
    durationMs: result ? 1500 : null,
    startedAt: 0,
    children: null,
    ...extra
  };
...
describe('tool cards', () => {
  ...
  // impossible-if-true: $ToolCall — an expanded card holds back part of its result behind anything but the cap the reader can lift
  it('the base card: head text, state, expansion on the chat, the cap and its lift', () => {
    ...
    const model = new ToolCall.Class({ call, chat, message: null });
  ...
  it('a shell call: command, stdout and stderr with ANSI stripped, the exit state', () => {
    ...
    const call = makeCall(
      'Bash',
      { command: 'npm test\necho done', description: 'Run the tests', run_in_background: true },
      {
        text: 'unused',
    ...
    const silent = new ToolCallBash.Class({
      call: makeCall(
        'Bash',
        { command: 'true' },
    ...
    const failed = new ToolCallBash.Class({
  ...
  it('an edit: a unified diff from hunks, or from the strings, with a change count', () => {
    ...
    const model = new ToolCallEdit.Class({
      call: makeCall(
        'Edit',
        { file_path: '~/dev/app.ts', old_string: 'b', new_string: 'c', replace_all: true },
        { structured: { structuredPatch: hunks } }
      ),
      chat,
      message: null
    });
    ...
    const plain = new ToolCallEdit.Class({
  ...
  it('a read: numbers come off the listing and the counter starts where the read started', () => {
    const { instance: chat, unmount } = chatHost();
    const model = new ToolCallRead.Class({
      call: makeCall(
        'Read',
        { file_path: 'lib/Reactive.ts', offset: 400, limit: 2 },
        {
          text: '   400\tconst a = 1;\n   401\tconst b = 2;',
          structured: { file: { numLines: 2, totalLines: 900 } }
        }
      ),
      chat,
      message: null
    ...
    const image = new ToolCallRead.Class({
  ...
  it('a write is the file with a line count; an agent folds its thread; an MCP call splits its name', () => {
    const { instance: chat, unmount } = chatHost();
    const write = new ToolCallWrite.Class({
      call: makeCall(
        'Write',
    ...
    const agent = new ToolCallAgent.Class({
      call: makeCall(
        'Agent',
        { description: 'survey', prompt: 'look', subagent_type: 'Explore' },
        { text: 'Findings.', structured: { resolvedModel: 'sonnet' } },
        { children: thread }
      ),
      chat,
      message: null
    ...
    const mcp = new ToolCallMcp.Class({
      call: makeCall(
