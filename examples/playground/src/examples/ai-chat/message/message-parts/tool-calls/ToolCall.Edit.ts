...
class $ToolCallEdit extends ToolCall.$Class {
  ...
  get hunks(): ToolCallEdit.Hunk[] {
    const patch = this.structured.structuredPatch;
    return Array.isArray(patch)
      ? (patch as ToolCallEdit.Hunk[]).filter((hunk) => Array.isArray(hunk.lines))
      : [];
  }
...
export namespace ToolCallEdit {
  export const $Class = $ToolCallEdit;
