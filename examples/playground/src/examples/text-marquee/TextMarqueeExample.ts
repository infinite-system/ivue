...
class $TextMarqueeExample {
  protected static readonly SENTENCES = [
    ...
    for (let chapter = 1; chapter <= 200; chapter++) {
      ...
      for (let paragraph = 0; paragraph < 3; paragraph++) {
        ...
        for (let sentence = 0; sentence < 8; sentence++) {
          sentences.push(
            this.SENTENCES[(chapter * 5 + paragraph * 3 + sentence) % this.SENTENCES.length]
          );
        }
  ...
  protected get self() {
    return this.constructor as typeof $TextMarqueeExample;
  }

  get bookText() {
    return this.self.$bookText;
  }
  get bookLengthLabel() {
    return this.bookText.length.toLocaleString();
  }
  ...
  get statsLabel() {
    return (
      `${this.bookLengthLabel} characters · ` +
...
export namespace TextMarqueeExample {
  export const $Class = Static($TextMarqueeExample); // anchor — it declares statics; children `extends` this
