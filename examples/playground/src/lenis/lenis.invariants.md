...
**Rejected alternatives:** The fractional write, which this record required until 2026-09-16. It rested on a trial that had rejected a snap — `renderSnap` in three flavours, `grid`, `fractional`, `auto` — but the trial was flawed twice over: the default flavour, `auto`, snapped only while the content moved a device pixel or more per frame and wrote the fraction at the slow tail, which is the one regime where the phase can be seen; and every iPhone run carried the Safari per-frame layer reset (`safariLayerReset`, see `setScroll`), a re-raster per frame that is a shimmer source of its own, so grid and fraction were never compared clean on that device. The earlier line-hop that had motivated the first snap was the row-height error (`wrapperScale` over a rounded `offsetHeight`), not the fraction; that finding stands. A snap of the target, or anywhere before the write: it discards the gesture's sub-pixel (see Mechanism).

**Evidence:** Judged by hand on 2026-09-16 at the slow tail of a glide, eyes on a text line, on an iPhone and a Galaxy S22 Ultra, fraction against device from the docs feel strip with the mounted rows and the heights unchanged: the device write is crisp, the fraction shimmers. The mounted-row work was cleared first — with a 200-row range held so a glide mounted nothing, the glide felt the same. Instrumented in Chrome at dpr 3 the write lands on thirds (100.4 → 100.333…) while the target keeps 100.4. Test: "the layer is written on the device-pixel grid, the model and the target untouched".
