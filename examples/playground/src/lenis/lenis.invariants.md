...
**Invariant:** If any decision is made from how fast the content is moving — the pad's lookahead, the brake's reach, a flick's fallback velocity, the creep's handoff — then the speed it reads is px per MILLISECOND, derived from the frame's real duration.
...
**Mechanism:** The layer is rasterised once, with its rows pixel-snapped by the browser relative to the layer, and the compositor places that raster at the transform. On the grid, the raster lands on device pixels and every line is crisp on every frame — where a browser's own scroll offset lands. At a fraction the compositor resamples: every line blurs by the fraction's phase, and as a glide slows the phase creeps from frame to frame, which reads as a shimmer at the slow tail and not at speed, where motion blur hides it. The model stays fractional so a finger's sub-pixel is never lost: a drag re-targets every frame from the finger's own position, and rounding the TARGET made the content walk under the thumb in whole steps.

**Rejected alternatives:**

- The fractional write — what this record required until 2026-09-16. It rested on a trial that had rejected a snap, and the trial was flawed twice over. Tried as `renderSnap` in three flavours (`grid`, `fractional`, `auto`), removed in `f14f6d30` (grep `snapRendered`); reopened, re-run as a live A/B from the docs feel strip and made the default in `c7bf34ae` (grep `snapToDevicePixel`).
  - The default flavour, `auto`, snapped only while the content moved a device pixel or more per frame and wrote the fraction at the slow tail — the one regime where the phase can be seen. Judged at flick speed, where motion blur hides it.
  - Every iPhone run carried the Safari per-frame layer reset (`safariLayerReset`, see the next record), a re-raster per frame that is a shimmer source of its own, so grid and fraction were never compared clean on that device.
...
- Instrumented in Chrome at dpr 3 the write lands on thirds (100.4 → 100.333…) while the target keeps 100.4. Test: "the layer is written on the device-pixel grid, the model and the target untouched".
...
**Invariant:** If the reading creep runs and the layer can take a Web Animation, then it hands the layer ONE linear animation between two fractional endpoints — from the model's position to the end of the content, or to a ten-minute cap, at constant speed — and writes the model only — mirroring, each creep frame, the value the compositor shows rather than integrating its own frame gap, so a stall or a hidden tab the model would clamp cannot leave it behind the run and the scenes drawn from it; a run that reaches its cap chains the next from its last value at its exact end on the document timeline with 700 ms to go; every stop — input, a direction change, the end of the content, `stop()` — adopts the value the compositor is showing and releases the layer; a speed change sets the run's playback rate in place — the multiple of the speed it was built at — and never ends the run, and the rate is announced so tracks alongside take it too. A flick's glide already on the layer is left alone until it ends.

**Scope:** `VirtualScrollerAutoplay.ts` `creepStep`, `feedCompositor`, `runFrom`, `releaseCreepCompositor`, `RUN_MS`, `CHAIN_AT_MS`; `Lenis.ts` `startCompositorSequence` (`linear`, `durationMs`, `after`), `setCompositorRate`, `rateOf`, `compositorShown`, `compositorPlaying`, `compositorHasChained`, `compositorRemainingMs`, `releaseCompositor`.
...
**Impossible if true:** A visible seam during a creep short of ten minutes. A creep frame written inline while a run plays. A run started while a flick's glide owns the layer. A run ended by a speed change. A model behind the run it is mirroring. A stage piece that begins anywhere but the run's start plus its offset, or moving at a rate other than the run's.

**Rejected alternatives:** 2-second chunks chained end to end on the document timeline (`f3759c0c` to `2431c358`) — exact in their values and seamless on the iPhone, a 1 px shift of the whole text layer at every boundary on the Galaxy, confirmed by the interval following the chunk length. Releasing the layer and starting a fresh run on a speed change (`d7e2b497`) — the release is an animation ending under the text layer, and read as a stutter on the Galaxy.
