...
**Scope:** `VirtualScrollerAutoplay.ts` `creepStep`, `feedCompositor`, `runFrom`, `releaseCreepCompositor`, `RUN_MS`, `CHAIN_AT_MS`; `Lenis.ts` `startCompositorSequence` (`linear`, `durationMs`, `after`), `setCompositorRate`, `rateOf`, `compositorPlaying`, `compositorHasChained`, `compositorRemainingMs`, `releaseCompositor`.
