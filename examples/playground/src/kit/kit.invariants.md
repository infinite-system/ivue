...
**Scope:** `Kit.Class.derive`, `resolve`, `view`, `merge`, `mergeEntry`, `freeze`. Every override at every depth, including one resolved from data at runtime. A role holds an entry and nothing else; a dispatch by an external name (`MessagePartToolCall`'s cards) is a flat set of roles picked by `roleOf`. A `subkit` arrives only through a patch — `mergeEntry` derives the child there; `resolve` is the one-line form for a subclass file that writes a `subkit` reach by hand, the form the malleability guide shows.
...
**Generates:** `defineProps(X.Class.props)` and the one `new` in every view; sections as roles (`Header`, `Body`, `Frame`); the `GroupedBody` fixture that renders from the model instead of the slot.
...
**Scope:** `Kit.Class.derive` over any kit with `order`; `merge`, `order`, `placing`, `apply`, `relationsOf`; the types `Kit.OrderPatch`, `Kit.Checked`, `Kit.Derived`. Below a `subkit` the added names are open to the types and the runtime check holds the line. The container's template is `<template v-for="role in model.kit.order" :key="role"><component :is="model.kit[role].view" v-bind="model.seam(role)" /></template>` and nothing else; presence is each leaf's own root `v-if` on a named getter of the model, which a layer overrides with a `super` fallback, never a bind. A markup leaf between roles is a tag role — an entry whose `view` is a tag name and has no `namespace`; a wrapper around a subset is a container role with an order of its own.
...
**Evidence:** `Kit.test.ts`: "seam() hands an unbound role { model, kit }, a bound class role its entry beside the bind, and a tag role only the bind", "two layers over a bound role compose through inherited, and the mounted items carry every layer", "presence is the leaf's own: its root carries a v-if on a named getter of the model, and a layer changes it by overriding the getter", "a typed entry refuses a wrong prop name and a subkit bind refuses a field its model lacks; the right names compile" (compile-time); `ChatMessage.test.ts`: "seam() hands a section the model and its entry, and a part its entry beside the bind — one method, no role named".
...
**Mechanism:** Every derived namespace carries the patch it is and, when `derive` was given one, a name (else `layer N` by its place in the chain); `writesOf` flattens a patch to keys — `Message.Header.view`, `Message#move:Footer` — and `conflicts` counts the layers per key over the whole chain, exempting `.bind` and inserted positions. `tree` computes each layer's keys once and hands the map down, so a tree over a chat-sized kit is linear in the patches, not quadratic. A class prints by its raw name: `Static()` names the class it wrapped on the bound subclass under its own `STATIC_RAW` key, and `derive`'s subclass is anonymous, so `className` unwraps the first and steps over the second until a named class stands — no class name inside `Static()` is known here. Nothing is merged: `mergeEntry` still spreads the patch over the base, so the report describes what happened and changes nothing.
...
**Invariant:** If a class composes others, then its kit declares exactly the roles its own template renders — an entry per role and an `order` when the template loops — and nothing below that level; the class an entry names declares the next level in its own kit; the tree is classes pointing at namespaces and is never written as one nested object; and a view that loops over seams is a compositor with a class of its own, never a classless leaf whose roles a parent declares for it. `KitContainer` is that class's base: the kit and `seam` inherited, `roleOf` and `keyOf` the two facts a list supplies, a dispatch supplying `roleOf` by an external name, `viewOf` and `propsOf` derived.

**Scope:** every compositor. `subkit` is the only place one kit mentions another's roles, and it edits them — a patch downward, never a declaration. The test for a child: if a view sits between the compositor and a role, the role belongs to that view, and it needs a class to declare it. An entry declares what is true of its role alone — view, namespace, props, bind; presence is neither's — a leaf's root carries `v-if` on a named getter of the model, so a layer changes it the way it changes any fact of the model, by overriding the getter; the container declares what is true of the list — `keyOf`, what identifies an item, and `roleOf`, which role an item takes, a total function with its own precedence and fallback that a layer overrides with a `super` fallback. Splitting dispatch across entries as per-entry predicates was tried and reverted: it scattered one function into many with implicit precedence.
...
**Rejected alternatives:** a `children` field on an entry — a third kind of thing under an entry, a branch in every walk, and `subkit` with the opposite meaning; a `Parts` map on the row holding the part roles — kept the row declaring roles it does not render; a role map under one key (`Tools`, the cards by tool name) — the last reason a kit held anything but entries, and a branch in `merge`, `freeze` and the inspector; a flat set of roles keyed by the same names dispatches through `roleOf` with no map; a per-class kit object carrying `propsOf` and friends — a seam needs the model, so either the template names it twice or an object is allocated per row, and the kit stops being data; mixin capabilities — the kit is the essence of a compositor, one base per essence, everything else held.
