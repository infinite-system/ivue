...
**Mechanism:** Every derived namespace carries the patch it is and, when `derive` was given one, a name (else `layer N` by its place in the chain); `writesOf` flattens a patch to keys — `Message.Head.view`, `Message#move:Foot` — and `conflicts` counts the layers per key over the whole chain, exempting `.bind` and inserted positions. Nothing is merged: `mergeEntry` still spreads the patch over the base, so the report describes what happened and changes nothing.
