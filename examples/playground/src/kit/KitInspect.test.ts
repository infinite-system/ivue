...
describe('a second write to one field is reported, never merged', () => {
  ...
  it('a view written twice warns with both layers in order, throws when strict, and the last write wins', () => {
    ...
    const One = Kit.Class.derive(Strip, { Head: { view: StripFootView } }, 'one');
    expect(KitInspect.Class.report(One)).toEqual([]);
    expect(warn).not.toHaveBeenCalled(); // the shipped base is not a layer
    const Two = Kit.Class.derive(One, { Head: { view: StripHeadView } }, 'two');
    expect(KitInspect.Class.report(Two)).toEqual(['Head.view: written by one, then two']);
    expect(warn).toHaveBeenCalledTimes(1);
    expect(String(warn.mock.calls[0][0])).toContain('Head.view: written by one, then two');
    expect(Two.$Class.$kit.Head.view).toBe(StripHeadView); // derive itself stays silent and the last write wins
    const App = Kit.Class.derive(Two, { Head: { view: StripFootView } }, 'app');
  ...
  it('a role moved twice is a contact, an insert beside another is not, a bind over a bind is not, and a nested write is keyed by its path', () => {
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
    const Moved = Kit.Class.derive(Strip, { order: { move: { Foot: { before: 'Head' } } } }, 'one');
    const MovedAgain = Kit.Class.derive(
      Moved,
      { order: { move: { Foot: { after: 'Body' } } } },
      'two'
    );
    expect(KitInspect.Class.report(MovedAgain)).toEqual(['#move:Foot: written by one, then two']);
    ...
    const Plain = Kit.Class.derive(
      Nested,
      { Card: { subkit: { Head: { view: CardHeadView } } } },
      'plain'
    );
    expect(KitInspect.Class.conflicts(Plain)).toEqual([
      'Card.Head.view: written by fancy, then plain'
    ]);
  });
