...
数字来自可复现的测量：引擎大小为 **1.1 kB gzip**，依赖数为 **0**，测试覆盖率为 **100%**；实例创建速度比对照方案快 **55–253 倍**；按键观察的模型可以表示 **20,000,000 个单元格**，每个单元格约 4.7 bytes。一个实际案例是 Invar：AI agents 在 ivue 上构建了约 **108,000 行**终端 IDE 源码，整个代码库只有一个 `computed()`。
